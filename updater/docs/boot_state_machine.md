...
The Sankofa updater manages over-the-air code updates for Flutter
...
`runtime/sankofa/patch_cache.cc`:
...
`shell/common/sankofa/sankofa.cc`, after the Dart VM is created
successfully.
...
`ConfigureSankofa()` during `FlutterMain::Init()`. This caused a bug:
