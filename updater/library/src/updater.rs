...
impl Display for InitError {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        match self {
            ...
            InitError::AlreadyInitialized => write!(f, "Sankofa has already been initialized."),
...
/// The yaml string is the contents of the `sankofa.yaml` file.
...
pub fn init(
    ...
    sankofa_debug!("libapp_path: {:?}", libapp_path);
...
pub fn handle_prior_boot_failure_if_necessary() -> Result<(), InitError> {
    ...
    .map_err(|e| {
        sankofa_error!("Failed to clean up after a failed patch: {:?}", e);
        InitError::FailedToCleanUpFailedPatch
    })
}
...
pub fn check_for_downloadable_update(channel: Option<&str>) -> anyhow::Result<bool> {
    ...
    sankofa_debug!("Patch check response: {:?}", response);
...
fn check_hash(path: &Path, expected_string: &str) -> anyhow::Result<()> {
    ...
    // https://github.com/sankofatech/updater/issues/56
    if !hash_matches {
        bail!(
...
fn update_internal(_: &UpdaterLockState, channel: Option<&str>) -> anyhow::Result<UpdateStatus> {
    ...
    for event in events {
        ...
        if let Err(err) = result {
            sankofa_error!("Failed to report event: {:?}", err);
        }
    }
    let request = with_mut_state(|state| {
        ...
        if let Err(err) = result {
            sankofa_error!("Failed to clear events: {:?}", err);
        }
    ...
    sankofa_info!("Patch check response: {:?}", response);
    ...
    match action {
        DownloadAction::Skip(SkipReason::KnownBad) => {
            sankofa_info!("Patch {} is known bad, skipping.", patch.number);
            return Ok(UpdateStatus::UpdateIsBadPatch);
        }
        DownloadAction::Skip(SkipReason::AlreadyInstalled) => {
            sankofa_info!("Patch {} is already installed, skipping.", patch.number);
            return Ok(UpdateStatus::NoUpdate);
        }
        _ => {}
    }
    ...
    if let Err(ref err) = result {
        ...
        if let Err(queue_err) = queue_result {
            sankofa_error!("Failed to queue update failure event: {:?}", queue_err);
        }
    }
    ...
    if !matches!(action, DownloadAction::Complete) {
        let resume_from = match action {
            DownloadAction::Resume { offset } => {
                sankofa_info!("Resuming download from byte {}", offset);
                offset
            }
            _ => 0,
        };
        ...
        sankofa_info!("Prior download already complete; skipping fetch.");
    }
...
fn install_downloaded_patch(
    ...
    if let Err(e) = check_hash(&installed_path, &patch.hash).with_context(|| {
        ...
        the version {} that was submitted to Sankofa.",
    ...
    with_mut_state(|state| {
        ...
        std::thread::spawn(move || {
            ...
            if let Err(err) = crate::network::send_patch_event(event, &config_clone) {
                sankofa_error!("Failed to report patch download: {:?}", err);
            }
        });
    ...
    RS: Read + Seek,
{
    ...
    sankofa_debug!("Base file size: {} bytes", base_total_size);
    ...
    match (patch_result, decompress_result) {
        (Ok(_), Ok(_)) => {
            ...
            sankofa_info!("Patch successfully applied to {:?}. {}", output_path, diag);
            Ok(())
        }
        (Err(patch_err), Ok(_)) => {
            sankofa_error!("Patch application failed; decompression OK. {}", diag);
...
/// surfaced over FFI as `sankofa_current_boot_patch_number`. `None`
...
pub fn report_launch_start() -> anyhow::Result<()> {
    sankofa_info!("Reporting launch start.");
...
pub fn report_launch_failure() -> anyhow::Result<()> {
    sankofa_info!("Reporting failed launch.");
    ...
    with_config(|config| {
        ...
        if mark_result.is_err() {
            sankofa_error!("Failed to mark patch as bad: {:?}", mark_result);
        }
        let client_id = state.client_id();
...
pub fn report_launch_success() -> anyhow::Result<()> {
    sankofa_info!("Reporting successful launch.");
    ...
    with_config(|config| {
        ...
        std::thread::spawn(move || {
            ...
            if let Err(err) = report_result {
                sankofa_error!("Failed to report successful patch install: {:?}", err);
            }
        });
...
pub fn start_update_thread() {
    std::thread::spawn(move || {
        ...
        let status = match result {
            Ok(status) => status,
            Err(err) => {
                sankofa_error!("Update failed: {:?}", err);
                UpdateStatus::UpdateHadError
            }
        };
        sankofa_info!("Update thread finished with status: {}", status);
    });
}
...
mod multi_engine_tests {
    ...
    /// - The C++ TryLoadFromPatch() in runtime/sankofa/patch_cache.cc uses std::once_flag
    /// - This ensures sankofa_report_launch_start() is only called once per process
