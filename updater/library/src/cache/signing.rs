...
pub fn check_signature(message: &str, signature: &str, public_key: &str) -> Result<()> {
    sankofa_debug!("Message is {}", message);
    sankofa_debug!("Public key is {:?}", public_key);
    sankofa_debug!("Signature is {}", signature);
    ...
    sankofa_info!("Verifying patch signature...");
    match public_key.verify(message.as_bytes(), &decoded_sig) {
        Ok(_) => {
            sankofa_info!("Patch signature is valid");
            Ok(())
        }
        Err(_) => {
            ...
            sankofa_error!("Patch signature is invalid");
