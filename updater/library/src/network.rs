...
pub fn patch_check_request_default(
    ...
    sankofa_info!("Sending patch check request: {:?}", request);
    ...
    sankofa_debug!("Patch check response: {:?}", parsed);
    Ok(parsed)
}
...
/// <https://docs.sankofa.dev/privacy>
/// The request body for the patch check endpoint.
#[derive(Debug, Serialize)]
pub struct PatchCheckRequest {
    /// The Sankofa app_id built into the sankofa.yaml in the app.
    /// app_ids are unique to each app and are used to identify the app
    /// within Sankofa's system (similar to a bundle identifier).  They
    /// are not secret and are safe to share publicly.
    /// <https://docs.sankofa.dev/concepts>
    pub app_id: String,
    /// The Sankofa channel built into the sankofa.yaml in the app.
    ...
    pub arch: String,
    /// The unique ID of this device. This is a random UUID generated by Sankofa and _not_ the
    /// device's UUID or any other identifier that has meaning outside of Sankofa.
...
pub fn download_to_path(
    ...
    sankofa_info!("Downloading patch from: {}", url);
