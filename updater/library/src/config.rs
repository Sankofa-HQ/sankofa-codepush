...
/// cbindgen:ignore
#[cfg(not(test))]
const DEFAULT_BASE_URL: &str = "https://api.sankofa.dev";
...
/// `sankofa_current_boot_patch_number`). Lives outside the on-disk
...
pub fn set_config(
    ...
    with_config_mut(|config: &mut Option<UpdateConfig>| {
        if config.is_some() {
            ...
            bail!("Updater already initialized, ignoring second sankofa_init call.");
        }
        ...
        sankofa_debug!("Updater configured with: {:?}", new_config);
...
// Arch/Platform names need to be kept in sync with the sankofa cli.
