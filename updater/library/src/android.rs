...
/// e.g. when input:
/// "/data/app/~~7LtReIkm5snW_oXeDoJ5TQ==/com.example.sankofa_test-rpkDZSLBRv2jWcc1gQpwdg==/lib/x86_64/libapp.so"
/// Will return:
/// "/data/app/~~7LtReIkm5snW_oXeDoJ5TQ==/com.example.sankofa_test-rpkDZSLBRv2jWcc1gQpwdg=="
...
fn find_and_open_lib(apks_dir: &Path, lib_name: &str) -> anyhow::Result<ZipLocation> {
        ...
        if let Some(filename) = path.file_name() {
            if let Some(filename) = filename.to_str() {
                ...
                if filename.ends_with(".apk") && filename.contains(arch.apk_split) {
                    sankofa_debug!("Checking APK: {:?}", path);
                    if let Ok(zip) = check_for_lib_path(&path, &lib_path) {
                        sankofa_debug!("Found lib in apk split: {:?}", path);
                        return Ok(zip);
                    }
                }
            }
    ...
    sankofa_debug!("Checking base APK: {:?}", base_apk_path);
...
pub(crate) fn open_base_lib(apks_dir: &Path, lib_name: &str) -> anyhow::Result<Box<dyn ReadSeek>> {
    ...
    // https://github.com/sankofatech/updater/pull/6
    ...
    sankofa_debug!("libapp.so is not stored uncompressed; falling back to buffered read");
...
pub fn libapp_path_from_settings(original_libapp_paths: &[String]) -> Result<PathBuf, InitError> {
    ...
    // https://github.com/sankofatech/sankofa/issues/283
    ...
    sankofa_debug!("Finding apk from: {:?}", full_libapp_path);
...
mod tests {
    ...
    fn app_data_dir_from_libapp_path_test() {
        let path = "/data/app/~~7LtReIkm5snW_oXeDoJ5TQ==/com.example.sankofa_test-rpkDZSLBRv2jWcc1gQpwdg==/lib/x86_64/libapp.so";
        let dir = super::app_data_dir_from_libapp_path(path).unwrap();
        assert_eq!(
            dir,
            std::path::PathBuf::from("/data/app/~~7LtReIkm5snW_oXeDoJ5TQ==/com.example.sankofa_test-rpkDZSLBRv2jWcc1gQpwdg==")
        );
    }
