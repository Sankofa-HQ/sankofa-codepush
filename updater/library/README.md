...
2. The user via the C API (using the `sankofa_code_push` package).
3. Network activity.
...
`patch_verification` in `sankofa.yaml`. Both modes require a
...
Strict mode is currently default for Sankofa, however some of our large
