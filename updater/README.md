...
This is the Rust side of the Sankofa code push system. This is built
in Rust with a C API for easy calling from other languages, most notably
for linking into `libflutter.so`.

The primary modification Sankofa makes to the stock Flutter engine
...
- `patch`: Developer tooling to package Sankofa updates ("patches"). Built
  into `patch.exe` and downloaded and run by `sankofa` command line tool:
  https://github.com/sankofatech/sankofa/tree/main/packages/sankofa_cli.
- `sankofa_code_push`: The Dart bindings for communicating with the updater
...
3.  In a second window, I open `code third_party/updater`. I do this because
    ...
    `sankofatech/buildroot` and is controlled via `gclient` by
    `sankofatech/engine/DEPS`.
...
```
sankofa run --local-engine-src-path $HOME/Documents/GitHub/engine/src \
