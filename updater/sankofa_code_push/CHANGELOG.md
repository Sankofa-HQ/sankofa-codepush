...
- **BREAKING**: revamp the updater API
  - Remove `SankofaCodePush` in favor of `SankofaUpdater`
...
- Remove confusing log message printed when Sankofa is not available.
...
- break: `package:sankofa_code_push/sankofa_code_push_io.dart` and
  `package:sankofa_code_push/sankofa_code_push_web.dart` have moved into
  `src/` to discourage accidental direct import of these files. Please import
  `package:sankofa_code_push/sankofa_code_push.dart` instead.
- Fixes repository link in pubspec.yaml

# 1.1.0

- feat: introduce `isSankofaAvailable` to determine whether the Sankofa Engine is detected
