...
void main() {
  group(Updater, () {
    ...
    group('currentPatchNumber', () {
      test('forwards the result of sankofa_current_boot_patch_number', () {
        when(
          () => updaterBindings.sankofa_current_boot_patch_number(),
        ).thenReturn(123);
    ...
    group('checkForDownloadableUpdate', () {
      test('forwards the result of sankofa_check_for_downloadable_update',
          () {
        when(
          () => updaterBindings.sankofa_check_for_downloadable_update(
            nullptr,
          ),
        ).thenReturn(true);
    ...
    group('nextPatchNumber', () {
      test('forwards the result of sankofa_next_boot_patch_number', () {
        when(
          () => updaterBindings.sankofa_next_boot_patch_number(),
        ).thenReturn(123);
    ...
    group('update', () {
      test('calls bindings.sankofa_update_with_result', () {
        when(
          () => updaterBindings.sankofa_update_with_result(nullptr),
        ).thenReturn(nullptr);
        updater.update();
        verify(
          () => updaterBindings.sankofa_update_with_result(nullptr),
    ...
    group('freeUpdateResult', () {
      test('calls bindings.sankofa_free_update_result', () {
        final result = calloc.allocate<UpdateResult>(sizeOf<UpdateResult>());
        updater.freeUpdateResult(result);
        verify(
          () => updaterBindings.sankofa_free_update_result(any()),
        ).called(1);
      });
    });
  });
