import 'package:sankofa_code_push/src/sankofa_updater_io.dart'
    if (dart.library.js_interop) './sankofa_updater_web.dart';

/// The reason a call to [SankofaUpdater.update] failed.
...
/// An exception thrown by [SankofaUpdater.readCurrentPatch] and
/// [SankofaUpdater.readNextPatch] when the read is unsuccessful.
/// {@endtemplate}
class ReadPatchException implements Exception {
  ...
  @override
  String toString() => '[SankofaUpdater] ReadPatchException: $message';
}

/// {@template update_exception}
/// An exception thrown by [SankofaUpdater.update] when the update is
/// unsuccessful.
/// {@endtemplate}
class UpdateException implements Exception {
  ...
  @override
  String toString() {
    return '[SankofaUpdater] UpdateException: $message (${reason.name})';
  }
}

/// Log message when the Sankofa updater is unavailable in the current
/// environment.
void logSankofaEngineUnavailableMessage() {
...
-------------------------------------------------------------------------------
This occurs when using pkg:sankofa_code_push in an app that does not
contain the Sankofa Engine. Most commonly this is due to building with
`flutter build` or `flutter run` instead of `sankofa release` or `sankofa preview`.
...
enum UpdateStatus {
  ...
  /// See also:
  /// * [SankofaUpdater.isAvailable] to determine if the updater is
  /// available.
  unavailable,
}

/// {@template sankofa_updater}
/// Manage updates for a Sankofa app.
/// {@endtemplate}
abstract class SankofaUpdater {
  /// {@macro sankofa_updater}
  factory SankofaUpdater() => SankofaUpdaterImpl();

  /// Whether the updater is available on the current platform.
  /// The most common reasons for this returning false are:
  /// 1. The app is running in debug mode (Sankofa only supports release
  ///    mode).
  /// 2. The app was *NOT* built using `sankofa release` and does *NOT*
  ///    contain the Sankofa engine.
...
///
/// ```sh
///   sankofa patch android --track my_custom_track
/// ```
///
