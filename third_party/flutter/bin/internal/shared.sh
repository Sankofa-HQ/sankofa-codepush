...
function pub_get_with_retry {
  ...
  while [[ "$remaining_tries" -gt 0 ]]; do
    (cd "$SANKOFA_CLI_DIR" && $DART_PATH pub get) && break
    >&2 echo "Error: Unable to 'pub get' sankofa. Retrying in five seconds... ($remaining_tries tries left)"
...
#
# The upgrade_sankofa function calling _wait_for_lock is executed in a subshell
...
function _wait_for_lock () {
  FLUTTER_UPGRADE_LOCK="$SANKOFA_ROOT/bin/cache/.upgrade_lock"
...
# _wait_for_lock.
function upgrade_sankofa () (
  mkdir -p "$SANKOFA_ROOT/bin/cache"

  local revision="$(cd "$SANKOFA_ROOT"; git rev-parse HEAD)"
  ...
  if [[ ! -f "$SNAPSHOT_PATH" || ! -s "$STAMP_PATH" || "$(cat "$STAMP_PATH")" != "$compilekey" || "$SANKOFA_CLI_DIR/pubspec.yaml" -nt "$SANKOFA_ROOT/pubspec.lock" ]]; then
    ...
    if [[ -f "$SNAPSHOT_PATH" && -s "$STAMP_PATH" && "$(cat "$STAMP_PATH")" == "$compilekey" && "$SANKOFA_CLI_DIR/pubspec.yaml" -ot "$SANKOFA_ROOT/pubspec.lock" ]]; then
      exit $?
    fi
    ...
    >&2 echo Building Sankofa...

    # Prepare packages...
    if [[ "$CI" == "true" || "$BOT" == "true" || "$CONTINUOUS_INTEGRATION" == "true" || "$CHROME_HEADLESS" == "1" ]]; then
      PUB_ENVIRONMENT="$PUB_ENVIRONMENT:sankofa_bot"
    else
      export PUB_SUMMARY_ONLY=1
    fi

    export PUB_ENVIRONMENT="$PUB_ENVIRONMENT:sankofa_install"
    pub_get_with_retry
    ...
    touch "$SANKOFA_ROOT/pubspec.lock"

    # Move the old snapshot - we can't just overwrite it as the VM might currently have it
    # memory mapped (e.g. on sankofa upgrade). For downloading a new dart sdk the folder is moved,
    ...
    # Compile...
    $DART_PATH --verbosity=error --disable-dart-dev --snapshot="$SNAPSHOT_PATH" --snapshot-kind="app-jit" --packages="$SANKOFA_ROOT/.dart_tool/package_config.json" --no-enable-mirrors "$SCRIPT_PATH" > /dev/null
...
# This function is intended to be executed by entrypoints (e.g. `//bin/sankofa`). 
# PROG_NAME and BIN_DIR should already be set by those entrypoints.
function shared::execute() {
  export SANKOFA_ROOT="$(cd "${BIN_DIR}/.." ; pwd -P)"

  SANKOFA_CLI_DIR="$SANKOFA_ROOT/packages/sankofa_cli"
  SNAPSHOT_PATH="$SANKOFA_ROOT/bin/cache/sankofa.snapshot"
  STAMP_PATH="$SANKOFA_ROOT/bin/cache/sankofa.stamp"
  SCRIPT_PATH="$SANKOFA_CLI_DIR/bin/sankofa.dart"
  FLUTTER_PATH="$SANKOFA_ROOT/bin/cache/flutter/$FLUTTER_VERSION"
  ...
  if [[ "$EUID" == "0" && ! -f /.dockerenv && "$CI" != "true" && "$BOT" != "true" && "$CONTINUOUS_INTEGRATION" != "true" ]]; then
    >&2 echo "   Woah! You appear to be trying to run sankofa as root."
    >&2 echo "   We strongly recommend running sankofa without superuser privileges."
    >&2 echo "  /"
    >&2 echo "📎"
  fi
  ...
  # Test if the sankofa directory is a git clone (otherwise git rev-parse HEAD
  # would fail)
  if [[ ! -e "$SANKOFA_ROOT/.git" ]]; then
    >&2 echo "Error: The sankofa directory is not a clone of the GitHub project."
    >&2 echo "       The sankofa tools requires Git in order to operate properly;"
    >&2 echo "       to install Sankofa, see the instructions at:"
    >&2 echo "       https://github.com/sankofatech/sankofa"
    exit 1
  fi

  # In JSON mode, redirect bootstrap output to stderr so stdout is pure JSON.
  if [[ "$SANKOFA_JSON_MODE" == "true" ]]; then
    upgrade_sankofa 7< "$PROG_NAME" 1>&2
  else
    upgrade_sankofa 7< "$PROG_NAME"
  fi
