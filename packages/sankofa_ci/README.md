...
```sh
dart pub global activate sankofa_ci
```

## Quick Start

```sh
sankofa_ci generate --dry-run            # review what it would write
sankofa_ci generate --required           # write + add a single aggregator check
```
...
| Command | Purpose |
|---|---|
| `generate` | Write `.github/workflows/sankofa_ci.yaml` (plus reusable workflows in `--style static`) |
...
activate`s `sankofa_ci`, runs `verify` as a sanity check, then runs
...
```sh
sankofa_ci generate --codecov-token-secret CODECOV_TOKEN
```
...
```sh
sankofa_ci verify --ignore e2e,other_package
```
