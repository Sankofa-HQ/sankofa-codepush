...
`sankofa_ci.yaml` is the workflow `sankofa_ci generate` produces for
...
sankofa_ci generate --repo-root . --dry-run
```

The workflow has two jobs. `setup` activates `sankofa_ci` on the
