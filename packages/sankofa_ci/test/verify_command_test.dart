...
void main() {
  setUpTempDir('sankofa_ci_verify_');
...
jobs:
  setup:
    ...
    steps:
      ...
      - run: sankofa_ci affected_packages --sdk dart
''');
    initGitRepo(tempDir);
  ...
  test('mention without marker does not count as dynamic coverage', () async {
    ...
    // happened to mention `sankofa_ci affected_packages` would be
    // treated as dynamic coverage. The marker comment makes intent
    // explicit.
    createPackage(tempDir, 'packages/foo', 'foo');
    _writeWorkflow(tempDir, 'ci.yaml', '''
name: CI
# Note: sankofa_ci affected_packages used to live here.
on: [push]
jobs:
  hello:
    runs-on: ubuntu-latest
    steps:
      - run: echo "see sankofa_ci affected_packages docs"
''');
    initGitRepo(tempDir);
...
jobs:
  setup:
    runs-on: ubuntu-latest
    steps:
      - run: sankofa_ci affected_packages
  cspell:
    runs-on: ubuntu-latest
    steps:
      - run: echo
