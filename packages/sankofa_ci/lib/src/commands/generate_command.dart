...
import 'package:sankofa_ci/src/action_versions.dart';
import 'package:sankofa_ci/src/commands/repo_root_option.dart';
import 'package:sankofa_ci/src/dependency_resolver.dart';
import 'package:sankofa_ci/src/flutter_version_resolver.dart';
import 'package:sankofa_ci/src/package_description.dart';
import 'package:sankofa_ci/src/package_slug.dart';
import 'package:sankofa_ci/src/repository_analyzer.dart';
import 'package:sankofa_ci/src/repository_description.dart';
  ...
  GenerateCommand({LatestMajorResolver? resolveLatestMajor})
    ...
    argParser
      ..addOption(
        ...
        defaultsTo: '.github/workflows/sankofa_ci.yaml',
      )
      ..addOption(
        'style',
...
resolves to `required`. Skip the flag and any package slug is fine.''',
  ...
  Future<int> run() async {
    ...
    // run `sankofa_ci update_actions` to bump them.
...
''');
  ...
  Map<String, String> _buildStaticFiles(
    ...
    if (hasDart) {
      files['.github/workflows/_sankofa_ci_dart.yaml'] =
    ...
    if (hasFlutter) {
      files['.github/workflows/_sankofa_ci_flutter.yaml'] =
  ...
  String _buildStaticMainYaml({
    ...
    final buffer = StringBuffer()
      ..write('''
# Generated by sankofa_ci --style static. Safe to edit.
# Run `sankofa_ci verify` to check for dep graph drift.
name: Sankofa CI
...
''');
    ...
    steps:
      ...
      - run: dart pub global activate sankofa_ci
      - name: Verify CI coverage
        run: sankofa_ci verify
...
''');
    ...
    for (final package in packages) {
      ...
      final reusable = isFlutter
          ? '_sankofa_ci_flutter.yaml'
          : '_sankofa_ci_dart.yaml';
      final slug = slugs[package]!;
...
''';
  ...
  String _buildDynamicYaml(
    ...
    final buffer = StringBuffer()
      ..write('''
# Generated by sankofa_ci. Safe to edit.
# Run `sankofa_ci verify` to check for dep graph drift.
# sankofa_ci-managed: dynamic
name: Sankofa CI
...
''');
    if (hasDart) {
      buffer.write(r'''
          DART=$(sankofa_ci affected_packages --sdk dart $EXTRA_ARGS)
          echo "dart_packages=$DART" >> $GITHUB_OUTPUT
''');
    }
    if (hasFlutter) {
      buffer.write(r'''
          FLUTTER=$(sankofa_ci affected_packages --sdk flutter $EXTRA_ARGS)
