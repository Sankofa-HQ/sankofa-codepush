...
/// consumer (sankofa_cli's `build_trace_summary.dart`).
///
/// Once a name lands in a shipped flutter or aot_tools, sankofa_cli
/// has to understand it across every version pin that ships with a
/// Sankofa release. So:
///
/// * **Never rename** a constant here. Add a new one and have sankofa
///   recognize both.
/// * **Don't reuse** a removed constant's value for a different meaning
///   for the same reason.
/// * The Groovy init script (`sankofa_trace_init.gradle` in the
...
///
/// Producers emit via [wireName]. Consumers (sankofa_cli) parse via
...
enum TraceCategory {
  ...
  /// Per-task events emitted by `sankofa_trace_init.gradle`.
...
/// `sankofa_trace_init.gradle`, emitted in each gradle_task event's
...
class TraceNames {
  ...
  /// sankofa matches with `startsWith`.
  ...
  /// Full span name sankofa matches is `"pod install: <phase>"`.
...
/// (sankofa_cli) matches the assembled span name against this enum's
/// [wireName]s.
