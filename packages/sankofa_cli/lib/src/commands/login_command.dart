...
/// `sankofa login`
/// Login as a new Sankofa user.
/// {@endtemplate}
class LoginCommand extends SankofaCommand {
  @override
  String get description => 'Login as a new Sankofa user.';
  ...
  Future<int> run() async {
    if (auth.isAuthenticated) {
      ...
      logger
        ...
        ..info(
          'Run ${lightCyan.wrap('sankofa logout')} to log out and try again.',
        );
    ...
    try {
      ...
      final consoleUri = Uri.https('console.sankofa.dev');
      logger
        ..err('''
We could not find a Sankofa account for ${error.email}.''')
        ..info(
...
🎉 ${lightGreen.wrap('Welcome to Sankofa! You are now logged in as <${auth.email}>.')}

🔑 Credentials are stored in ${lightCyan.wrap(auth.credentialsFilePath)}.
🚪 To logout use: "${lightCyan.wrap('sankofa logout')}".''');
    return ExitCode.success.code;
  }
  ...
  void prompt(String url) {
    logger.info('''
The Sankofa CLI needs your authorization to manage apps, releases, and patches on your behalf.
