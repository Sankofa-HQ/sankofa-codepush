...
/// `sankofa preview` command.
/// {@endtemplate}
class PreviewCommand extends SankofaCommand {
...
This is only applicable when previewing Android releases.''',
  ...
  Future<int> run() async {
    ...
    final flavors = sankofaYaml?.flavors;

    if (results.wasParsed('app-id')) {
      appId = results['app-id'] as String;
    } else if (sankofaYaml != null && flavors == null) {
  ...
  Future<int> installAndLaunchMacos({
    ...
    // 2025-01-31 09:53:22.889 Df macos_sandbox[22535:1d268] [dev.sankofa:updater::cache::patch_manager] [sankofa] No public key provided, skipping signature verification
...
Skipping the ios-deploy fallback because it does not support iOS 17 or later.''',
  ...
  /// Sets the channel property in the sankofa.yaml file inside the Windows
  /// app whose root directory is [appDirectory].
  Future<void> setChannelOnWindowsApp({
    required Directory appDirectory,
    required String channel,
  }) async {
    final sankofaYamlFile = File(
      p.join(appDirectory.path, 'data', 'flutter_assets', 'sankofa.yaml'),
    );

    await _maybeSetChannelInSankofaYaml(
      channel: channel,
      sankofaYamlFile: sankofaYamlFile,
    );
  }

  /// Sets the channel property in the sankofa.yaml file inside the Runner.app
  Future<void> setChannelOnRunner({
    required Directory runnerDirectory,
    required String channel,
  }) async {
    await Isolate.run(() async {
      final sankofaYaml = File(
        p.join(
      ...
      if (!sankofaYaml.existsSync()) {
        throw Exception('Unable to find sankofa.yaml');
      }

      await _maybeSetChannelInSankofaYaml(
        channel: channel,
        sankofaYamlFile: sankofaYaml,
      );
    });
  }

  /// Sets the channel property in the sankofa.yaml file inside a macOS app.
  ...
  /// Unzips the `.aab` and sets the channel property in the sankofa.yaml
  ...
  Future<void> setChannelOnAab({
    ...
    await Isolate.run(() async {
      ...
      final didUpdateSankofaYaml = await _maybeSetChannelInSankofaYaml(
        channel: channel,
        sankofaYamlFile: sankofaYamlFile,
      );

      if (!didUpdateSankofaYaml) {
        return;
      }
  ...
  /// Sets the channel property in the sankofa.yaml file if none is set.
  Future<void> setChannelOnLinuxApp({
    required String channel,
    required Directory bundleDirectory,
  }) async {
    final sankofaYamlFile = File(
      p.join(bundleDirectory.path, 'data', 'flutter_assets', 'sankofa.yaml'),
    );
  ...
  /// Sets the channel property in the sankofa.yaml file if none is set or
  /// different from the provided channel.
  static Future<bool> _maybeSetChannelInSankofaYaml({
    required String channel,
    required File sankofaYamlFile,
  }) async {
    if (!sankofaYamlFile.existsSync()) {
      throw Exception('Unable to find sankofa.yaml');
    }

    final yamlText = sankofaYamlFile.readAsStringSync();
    ...
    sankofaYamlFile.writeAsStringSync(yamlEditor.toString(), flush: true);
    return true;
  }
