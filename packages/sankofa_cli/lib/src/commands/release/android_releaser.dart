...
To change the version of this release, change your app's version in your pubspec.yaml.''',
    ...
    if (generateApk && splitApk) {
      logger
        ..err(
          'Sankofa does not support the split-per-abi option at this time',
        )
        ..info(
          '''
...
Please comment and upvote ${link(uri: Uri.parse('https://github.com/sankofatech/sankofa/issues/1141'))} if you would like sankofa to support this.''',
        );
  ...
  Future<String> getReleaseVersion({
    ...
    try {
      releaseVersion = await sankofaAndroidArtifacts
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    final aabFile = sankofaAndroidArtifacts.findAab(
      project: projectRoot,
      flavor: flavor,
    );
  ...
  String get postReleaseInstructions {
    ...
    if (generateApk) {
      final apkFile = sankofaAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
      apkText = generateApk
