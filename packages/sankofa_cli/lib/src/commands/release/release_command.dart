...
import 'package:sankofa_cli/src/commands/release/release.dart';
...
import 'package:sankofa_cli/src/extensions/string.dart';
import 'package:sankofa_cli/src/logging/logging.dart';
import 'package:sankofa_cli/src/metadata/metadata.dart';
...
/// {@endtemplate}
class ReleaseCommand extends SankofaCommand {
...
Defaults to "latest" which builds using the latest stable Flutter version.''',
      ...
      // Added for https://github.com/sankofatech/sankofa/issues/3223.
...
of the iOS app that is using this module. (aar and ios-framework only)''',
  ...
  String get description =>
      'Creates a sankofa release for the provided target platforms.';
  ...
  /// The sankofa app ID for the current project.
  String get appId => sankofaEnv.getSankofaYaml()!.getAppId(flavor: flavor);
  ...
  /// The workflow to create a new release for a Sankofa app.
  ///
  ...
  Future<void> createRelease(Releaser releaser) async {
    ...
    try {
      await sankofaValidator.validateFlavors(
        flavorArg: flavor,
    ...
    try {
      await sankofaFlutter.installRevision(revision: targetFlutterRevision);
    } on Exception {
    ...
    final releaseFlutterSankofaEnv = sankofaEnv.copyWith(
    ...
    return await runScoped(
      () async {
        ...
        final flutterVersionString = await sankofaFlutter
...
✅ Published Release ${release.version}!''')
  ...
  Future<void> assertArgsAreValid(Releaser releaser) async {
    ...
    if (sankofaYaml?.patchVerification != null && !hasPublicKey) {
      logger.warn(
        'patch_verification is set in sankofa.yaml but '
    ...
    final version = await sankofaFlutter.resolveFlutterVersion(
      flutterVersionArg,
    );
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
  ...
  /// [sankofaEnv]. Will exit with [ExitCode.software] if the version
  /// specified by the user is not found/supported.
  Future<String> resolveTargetFlutterRevision() async {
    if (flutterVersionArg == 'latest') return sankofaEnv.flutterRevision;

    // Fetch the latest remote refs so that release branch pointers
    // (e.g. flutter_release/3.38.5) are up to date.
    await sankofaFlutter.fetchRemoteRefs();

    final String? revision;
    try {
      revision = await sankofaFlutter.resolveFlutterRevision(
        flutterVersionArg,
      );
...
$error''');
    ...
    if (revision == null) {
      final openIssueLink = link(
        uri: Uri.parse(
          'https://github.com/sankofatech/sankofa/issues/new?assignees=&labels=feature&projects=&template=feature_request.md&title=feat%3A+',
        ),
        message: 'open an issue',
      );
      logger.err('''
Version $flutterVersionArg not found. Please $openIssueLink to request a new version.
Use `sankofa flutter versions list` to list available versions.
''');
      throw ProcessExit(ExitCode.software.code);
    }
  ...
  Future<void> ensureVersionIsReleasable({
    ...
    if (existingRelease != null) {
      ...
      if (existingRelease.flutterRevision != flutterRevision) {
        final flutterVersion = await sankofaFlutter.getVersionForRevision(
          flutterRevision: flutterRevision,
        );

        final formattedCurrentReleaseVersion = sankofaFlutter.formatVersion(
          revision: flutterRevision,
          version: flutterVersion,
        );

        final formattedExistingReleaseVersion = sankofaFlutter.formatVersion(
...
To resolve this issue, you can:
  ...
  * Bump the release version and re-run the release command with the desired Flutter version.''',
  ...
  Future<void> printReleaseSummary({
    ...
    final flutterVersionString = await sankofaFlutter.getVersionAndRevision();
...
''');
    ...
    if (confirm && sankofaEnv.canAcceptUserInput) {
  ...
  Future<Release> getOrCreateRelease({
    ...
    return await codePushClientWrapper.maybeGetRelease(
        ...
        await codePushClientWrapper.createRelease(
          appId: appId,
          version: version,
          flutterRevision: sankofaEnv.flutterRevision,
          platform: releasePlatform,
        );
  }
  ...
  Future<void> finalizeRelease({
    ...
    final baseMetadata = UpdateReleaseMetadata(
      ...
      environment: BuildEnvironmentMetadata(
        ...
        sankofaVersion: packageVersion,
        sankofaYaml: sankofaEnv.getSankofaYaml()!,
        usesSankofaCodePushPackage: sankofaEnv.usesSankofaCodePushPackage,
      ),
      // Attach the build-trace summary if the build produced one.
      // Null for older Flutter pins without the --sankofa-trace flag
