...
import 'package:sankofa_cli/src/commands/release/apple_releaser_mixin.dart';
...
If left checked, Xcode will rewrite the build number in the uploaded IPA, so the version that ships to App Store Connect will not match the version Sankofa recorded for this release. Patches will then fail to apply.''',
        );
    }

    // Delete the Sankofa supplement directory if it exists.
    // This is to ensure that we don't accidentally upload stale artifacts
    // when building with older versions of Flutter.
    final sankofaSupplementDir = artifactManager
        .getIosReleaseSupplementDirectory();
    if (sankofaSupplementDir?.existsSync() ?? false) {
      sankofaSupplementDir!.deleteSync(recursive: true);
    }
  ...
  Future<void> uploadReleaseArtifacts({
    ...
    await codePushClientWrapper.createIosReleaseArtifacts(
      ...
      podfileLockHash: sankofaEnv.iosPodfileLockHash,
    );
