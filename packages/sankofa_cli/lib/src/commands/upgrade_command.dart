...
/// `sankofa upgrade`
/// A command which upgrades your copy of Sankofa.
/// {@endtemplate}
class UpgradeCommand extends SankofaCommand {
  /// {@macro upgrade_command}
  UpgradeCommand();

  @override
  String get description => 'Upgrade your copy of Sankofa.';
  ...
  Future<int> run() async {
    ...
    try {
      currentVersion = await sankofaVersion.fetchCurrentGitHash();
    ...
    try {
      latestVersion = await sankofaVersion.fetchLatestGitHash();
    } on ProcessException catch (error) {
      updateCheckProgress.fail();
    ...
    if (isUpToDate) {
      logger.info('Sankofa is already at the latest version.');
    ...
    try {
      await sankofaVersion.attemptReset(revision: latestVersion);
    } on ProcessException catch (error) {
      updateProgress.fail();
