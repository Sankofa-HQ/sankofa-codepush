...
import 'package:sankofa_cli/src/archive/archive.dart';
...
import 'package:sankofa_cli/src/commands/patch/apple_patcher_mixin.dart';
import 'package:sankofa_cli/src/commands/patch/patcher.dart';
...
class IosPatcher extends Patcher
  ...
  String get _aotOutputPath =>
      p.join(sankofaEnv.buildDirectory.path, 'out.aot');

  String get _vmcodeOutputPath =>
      p.join(sankofaEnv.buildDirectory.path, 'out.vmcode');

  String get _appDillCopyPath =>
      p.join(sankofaEnv.buildDirectory.path, 'app.dill');
  ...
  @override
  String? get localPodfileLockHash => sankofaEnv.iosPodfileLockHash;
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}''');
  ...
  Future<Map<Arch, PatchArtifactBundle>> createPatchArtifacts({
    ...
    final useLinker = AotTools.usesLinker(sankofaEnv.flutterRevision);
    if (useLinker) {
      apple.copySupplementFilesToSnapshotDirs(
        ...
        patchSupplementDir: sankofaEnv.iosSupplementDirectory,
        patchSnapshotDir: sankofaEnv.buildDirectory,
      );
      ...
      final result = await apple.runLinker(
        ...
        ddMaxBytes: int.tryParse(
          platform.environment['SANKOFA_PATCH_DD_MAX_BYTES'] ?? '',
        ),
      );
    ...
    if (useLinker && await aotTools.isGeneratePatchDiffBaseSupported()) {
      ...
      final analyzeSnapshotPath = sankofaArtifacts.getArtifactPath(
