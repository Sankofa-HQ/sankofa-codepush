...
import 'package:sankofa_cli/src/commands/patch/patch.dart';
...
import 'package:sankofa_cli/src/formatters/formatters.dart';
...
import 'package:sankofa_cli/src/release_chooser.dart';
...
/// A command that creates a sankofa patch for the provided target platforms.
/// `sankofa patch --platforms=android,ios`
/// {@endtemplate}
class PatchCommand extends SankofaCommand {
  /// {@macro patch_command}
...
NOTE: this is ${styleBold.wrap('not')} recommended. Asset changes cannot be included in a patch can cause your app to behave unexpectedly.''';
  ...
  String get description =>
      'Creates a sankofa patch for the provided target platforms.';
  ...
  Future<void> createPatch(Patcher patcher) async {
    ...
    } else if (sankofaEnv.canAcceptUserInput) {
...
+-------------------------------------------------------------------------------+
''');
      lastBuiltFlutterRevision = sankofaEnv.flutterRevision;
    ...
    try {
      await sankofaFlutter.installRevision(revision: release.flutterRevision);
    } on Exception {
    ...
    if (obfuscationMapFile != null) {
      ...
      final shouldPreStripInGenSnapshot = await sankofaFlutter
    ...
    if (hasObfuscate && !hasSplitDebugInfo) {
      extraBuildArgs.add(
        '--split-debug-info=${p.join('build', 'sankofa', 'symbols')}',
      );
    }
        ...
        final baseMetadata = CreatePatchMetadata(
          ...
          environment: BuildEnvironmentMetadata(
            ...
            usesSankofaCodePushPackage:
                sankofaEnv.usesSankofaCodePushPackage,
          ),
          // Attach the build-trace summary if the build produced one.
          // Null for older Flutter pins without the --sankofa-trace
...
✓ Patch published to release ${release.version} on track ${track.channel}.
  ...
  void assertReleaseContainsPlatform({
    ...
    if (!contains) {
      ...
      logger.err(
        '''No release exists for $platformName in release version ${release.version}. Please run sankofa release $platformName to create one.''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
