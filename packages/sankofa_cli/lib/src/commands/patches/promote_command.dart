...
/// {@endtemplate}
class PromoteCommand extends SankofaCommand {
  ...
  Future<int> run() async {
    ...
    if (isJsonMode) {
      emitJsonError(
        ...
        message:
            'sankofa patches promote is deprecated and does not support '
            '--json output.',
        hint: 'Use `sankofa patches set-track --track=stable` instead.',
      );
      return ExitCode.usage.code;
    }

    logger.warn(
      '''This command is deprecated and will be removed in a future release. Use `sankofa patches set-track --track=stable` instead.''',
    );
    ...
    final appId = sankofaEnv.getSankofaYaml()!.getAppId(flavor: flavor);
...
This is a bug and should never happen. Please file an issue at https://github.com/sankofatech/sankofa/issues/new?assignees=&labels=bug&projects=&template=bug_report.md&title=fix%3A+''',
      );
