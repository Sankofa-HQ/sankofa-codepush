...
/// {@template cache}
/// A class that manages the artifacts cached by Sankofa.
...
/// [SankofaArtifacts] since uses the current Sankofa environment.
/// {@endtemplate}
class Cache {
  /// {@macro cache}
  Cache() {
  ...
  Directory getArtifactDirectory(String name) {
    return Directory(
      p.join(sankofaArtifactsDirectory.path, p.withoutExtension(name)),
    );
  }
  ...
  Directory getPreviewDirectory(String name) {
    return Directory(
      p.join(sankofaPreviewsDirectory.path, p.withoutExtension(name)),
    );
  }

  /// The Sankofa cache directory.
  static Directory get sankofaCacheDirectory {
    return Directory(p.join(sankofaEnv.sankofaRoot.path, 'bin', 'cache'));
  }

  /// The Sankofa cached previews directory.
  static Directory get sankofaPreviewsDirectory {
    return Directory(p.join(sankofaCacheDirectory.path, 'previews'));
  }

  /// The Sankofa cached artifacts directory.
  static Directory get sankofaArtifactsDirectory {
    return Directory(p.join(sankofaCacheDirectory.path, 'artifacts'));
  }
  ...
  Future<void> clear() async {
    final cacheDir = sankofaCacheDirectory;
...
/// An artifact which is cached by Sankofa.
/// {@endtemplate}
abstract class CachedArtifact {
  ...
  /// Whether the artifact is required for Sankofa to function.
...
If you're behind a firewall/proxy, please, make sure sankofa_cli is
...
class PatchArtifact extends CachedArtifact {
  ...
  Future<bool> _supportsArm64Patch() async {
    ...
    final version = await sankofaFlutter.resolveFlutterVersion(revision);
