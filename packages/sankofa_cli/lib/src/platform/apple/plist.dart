...
import 'package:sankofa_cli/src/platform/apple/invalid_export_options_plist_exception.dart';
...
class Plist {
  ...
  /// number on App Store Connect. This breaks Sankofa, because the build
  /// number that ships will not match the one Sankofa recorded for the
  /// release.
  ///
...
/// ships to App Store Connect will not match the build number Sankofa
...
Xcode will rewrite the build number in the exported IPA, so the version that ships to App Store Connect will not match the version Sankofa recorded for this release. Patches will fail to apply.
