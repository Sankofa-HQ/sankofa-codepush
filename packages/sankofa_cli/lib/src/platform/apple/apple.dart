...
import 'package:sankofa_cli/src/platform/apple/apple_platform.dart';
import 'package:sankofa_cli/src/platform/apple/link_result.dart';
import 'package:sankofa_cli/src/platform/apple/missing_xcode_project_exception.dart';
...
class Apple {
  ...
  // `aot_tools` rather than having to know the contents within `sankofa`.
  void copySupplementFilesToSnapshotDirs({
    ...
    // use the same supplement files. At the `sankofa` level we don't know
  ...
  Set<String>? flavors({required ApplePlatform platform}) {
    ...
    // https://github.com/sankofatech/sankofa/issues/1703 for an example.
  ...
  Future<LinkResult> runLinker({
    ...
    final buildDirectory = sankofaEnv.buildDirectory;
    ...
    final genSnapshot = sankofaArtifacts.getArtifactPath(
