...
import 'package:sankofa_cli/src/auth/ci_token.dart';
import 'package:sankofa_cli/src/auth/endpoints/endpoints.dart';
import 'package:sankofa_cli/src/auth/sankofa_oauth.dart' as sankofa_oauth;
...
/// The environment variable that holds the Sankofa CI token.
const sankofaTokenEnvVar = 'SANKOFA_TOKEN';
...
/// Callback for obtaining Sankofa access credentials via loopback login.
...
class AuthenticatedClient extends http.BaseClient {
  ...
  Future<oauth2.AccessCredentials> _refreshForProvider(
    ...
    try {
      // Sankofa uses its own refresh flow; Google and Microsoft use the
      // standard OAuth refresh. This branching can be removed once the
      // Google/Microsoft providers are fully removed from the CLI.
      if (authProvider == AuthProvider.sankofa) {
        return await sankofa_oauth.refreshSankofaCredentials(
          credentials,
          _baseClient,
...
class Auth {
  ...
  Auth({
    ...
    CodePushClientBuilder? buildCodePushClient,
       ...
       _authServiceUri = authServiceUri ?? sankofaEnv.authServiceUri,
       _obtainCredentialsViaLoopbackLogin =
           obtainCredentialsViaLoopbackLogin ??
           sankofa_oauth.obtainCredentialsViaLoopbackLogin,
  ...
  /// Logs in the user via the Sankofa loopback OAuth flow.
  Future<void> login({required void Function(String) prompt}) async {
    ...
    try {
      ...
      final codePushClient = _buildCodePushClient(
        ...
        hostedUri: sankofaEnv.hostedUri,
      );
  ...
  /// Logs out the user.
  ///
  /// If a Sankofa refresh token is available, revokes the server-side
  ...
  void _loadCredentials() {
    final envToken = platform.environment[sankofaTokenEnvVar];
    if (envToken != null) {
      ...
      logger.detail('[env] $sankofaTokenEnvVar parsed as legacy CiToken');
      return;
    }
...
extension OauthAuthProvider on Jwt {
  ...
  AuthProvider get authProvider {
    if (payload.iss == sankofaEnv.jwtIssuer) {
      return AuthProvider.sankofa;
...
extension OauthValues on AuthProvider {
  ...
  oauth2.AuthEndpoints get authEndpoints => switch (this) {
    ...
    (AuthProvider.sankofa) => throw UnsupportedError(
      'Sankofa auth does not use OAuth endpoints',
    ),
  };
    ...
    switch (this) {
      case AuthProvider.google:
        return oauth2.ClientId(
          /// Sankofa CLI's OAuth 2.0 identifier for GCP,
          '''523302233293-eia5antm0tgvek240t46orctktiabrek.apps.googleusercontent.com''',

          /// Sankofa CLI's OAuth 2.0 secret for GCP.
          ///
      ...
      case AuthProvider.microsoft:
        return oauth2.ClientId(
          /// Sankofa CLI's OAuth 2.0 identifier for Azure/Entra.
          '4fc38981-4ec4-4bd9-a755-e6ad9a413054',
        );
      case AuthProvider.sankofa:
        throw UnsupportedError('Sankofa auth does not use a client ID');
    }
  }
  ...
  List<String> get scopes => switch (this) {
    ...
    // Sankofa auth doesn't use scopes.
    (AuthProvider.sankofa) => [],
  };
}
