...
/// Exception thrown when the Sankofa auth flow fails.
class SankofaAuthException implements Exception {
  /// Creates a [SankofaAuthException] with the given [message].
  const SankofaAuthException(this.message);

  /// The error message.
  final String message;

  @override
  String toString() => 'SankofaAuthException: $message';
}

/// Implements the full loopback login flow for Sankofa auth.
///
...
Future<HttpRequest> _waitForCallback(
  ...
  try {
    return await completer.future.timeout(
      timeout,
      onTimeout: () {
        throw const SankofaAuthException(
...
/// callback [request].
///
/// Throws [SankofaAuthException] if the callback contains an error or is
...
/// Refreshes Sankofa tokens using the refresh token.
///
...
Future<oauth2.AccessCredentials> refreshSankofaCredentials(
  ...
  if (refreshToken == null) {
    throw const SankofaAuthException('No refresh token available.');
  }
...
/// issuer matches the expected JWT issuer from [SankofaEnv].
///
/// Expected JSON shape:
/// ```json
/// {
...
oauth2.AccessCredentials _parseTokenResponse(String responseBody) {
  ...
  try {
    ...
    throw SankofaAuthException('Invalid access token: ${e.message}');
  }

  // Validate the issuer matches the expected auth service.
  final expectedIssuer = sankofaEnv.jwtIssuer;
