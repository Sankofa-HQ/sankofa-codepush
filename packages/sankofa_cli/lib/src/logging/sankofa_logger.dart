...
final loggerRef = create(SankofaLogger.new);

/// The [Logger] instance available in the current zone.
SankofaLogger get logger => read(loggerRef);

const _logFileName = 'sankofa.log';

/// Where logs are written for the current Sankofa CLI run. A new file will
/// be created for every run of the Sankofa CLI, and will have the name
/// `timestamp_sankofa.log`.
final File currentRunLogFile = (() {
  final timestamp = clock.now().millisecondsSinceEpoch;
  final file = File(
    p.join(sankofaEnv.logsDirectory.path, '${timestamp}_$_logFileName'),
  );
...
/// {@endtemplate}
class SankofaLogger extends Logger {
  /// {@macro sankofa_logger}
  SankofaLogger({super.level});
  ...
  void _failIfNonInteractive({String? promptText, String? hint}) {
    if (isInteractive && sankofaEnv.canAcceptUserInput) return;
...
class _StaticProgress implements Progress {
  ...
  // No sankofa code path calls cancel(); required by the Progress
