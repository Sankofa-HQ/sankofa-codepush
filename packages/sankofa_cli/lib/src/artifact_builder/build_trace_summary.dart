...
// `sankofa_build_trace`'s `TraceSchema`; if you add a new bucket
...
import 'package:sankofa_cli/src/artifact_builder/duration_distribution.dart';

/// Summary of a Chrome Trace Event Format build trace.
///
/// Aggregate [Duration] timings and small integer counters, suitable
/// for uploading to Sankofa's servers as part of release telemetry.
class BuildTraceSummary {
  ...
  factory BuildTraceSummary.fromEvents(
    ...
    return _buildSummary(
      acc,
      platform: platform,
      sankofaOverhead: sankofaOverhead,
  ...
  static BuildTraceSummary _buildSummary(
    ...
    return BuildTraceSummary(
      platform: platform,
      total: flutterBuild + (sankofaOverhead ?? Duration.zero),
  ...
  /// Sankofa overhead computed from it) should parse once and pass the
  ...
  /// Total command wall-clock (Flutter build + Sankofa overhead).
  ...
  /// Wall-clock time the Sankofa CLI spent around Flutter. Null when
  /// the caller couldn't compute it.
  final Duration? sankofaOverhead;

  /// Network I/O time and request counts, summed across Sankofa-side
  ...
  /// Sankofa CLI's wall-clock time around Flutter with network I/O
  /// subtracted — i.e. what Sankofa spent doing local work (file I/O,
  /// hashing, archive assembly, aot_tools link/gen_snapshot bookkeeping
  /// outside their own spans). Null when [sankofaOverhead] is null.
  ///
  ...
  /// overhead).
  Duration? get sankofaLocal {
    final overhead = sankofaOverhead;
  ...
  Map<String, Object?> toJson() => <String, Object?>{
    ...
    'sankofaOverheadMs': sankofaOverhead?.inMilliseconds,
    'sankofaLocalMs': sankofaLocal?.inMilliseconds,
...
/// Network I/O totals. Combined across Sankofa-side (auth, artifact
...
/// `sankofa_trace_init.gradle` TaskExecutionListener.
class GradleStats {
