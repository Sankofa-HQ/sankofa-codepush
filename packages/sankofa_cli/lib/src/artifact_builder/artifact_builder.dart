...
import 'package:sankofa_cli/src/artifact_builder/build_environment.dart';
import 'package:sankofa_cli/src/artifact_builder/build_trace_session.dart';
import 'package:sankofa_cli/src/artifact_builder/build_trace_summary.dart';
...
import 'package:sankofa_cli/src/os/operating_system_interface.dart';
import 'package:sankofa_cli/src/platform.dart' as scoped_platform;
...
/// Used to wrap code that invokes `flutter build` with Sankofa's fork of
/// Flutter.
typedef SankofaBuildCommand = Future<void> Function();
...
If the above command fails, then this is likely not a Sankofa issue and
the underlying `flutter build` failure must be resolved for Sankofa to
build a release.
...
Sankofa will default to the latest stable version of Flutter.
We strongly encourage always specifying an explicit Flutter version:

${lightCyan.wrap('sankofa release <platform> --flutter-version=3.29.0')}

• If `flutter build` completes successfully and `sankofa release`
fails when using the same flutter version, please file an issue:
${link(uri: Uri.parse('https://github.com/sankofatech/sankofa/issues/new'))}
''';

  /// Cache of `flutter build <command>` help output checks for
  /// `--sankofa-trace` support. Populated lazily by
  /// [_supportsTraceFlag].
  final _traceSupport = <String, bool>{};

  /// Returns whether `flutter build <command>` accepts `--sankofa-trace`.
  ///
  ...
  Future<bool> _supportsTraceFlag(String command) async {
    ...
    try {
      ...
      final supported = result.stdout.toString().contains('--sankofa-trace');
  ...
  /// Returns the `--sankofa-trace` argument if the current
  ...
  Future<List<String>> _traceArgs(String command) async {
    ...
    return ['--sankofa-trace=${traceFile.path}'];
  }
  ...
  Future<File> buildAppBundle({
    ...
    await _runSankofaBuildCommand(() async {
...
Reason: Exited with code $exitCode.''',
    ...
    try {
      return sankofaAndroidArtifacts.findAab(
        project: projectRoot,
        flavor: flavor,
      );
  ...
  Future<File> buildApk({
    ...
    await _runSankofaBuildCommand(() async {
      ...
      final arguments = [
        ...
        // See https://github.com/sankofatech/sankofa/issues/1141.
...
Reason: Exited with code $exitCode.''',
    ...
    final projectRoot = sankofaEnv.getSankofaProjectRoot()!;
    try {
      return sankofaAndroidArtifacts.findApk(
        project: projectRoot,
        flavor: flavor,
      );
    } on MultipleArtifactsFoundException catch (error) {
  ...
  Future<void> buildAar({
    ...
    return _runSankofaBuildCommand(() async {
      const executable = 'flutter';
      final targetPlatformArgs = targetPlatforms?.targetPlatformArg;
      final arguments = [
        'build',
...
Reason: Exited with code $exitCode.''',
  ...
  /// --release` with Sankofa's fork of Flutter.
...
Reason: Exited with code $exitCode.''',
    ...
    if (appDillPath == null) {
      throw ArtifactBuildException(
        ...
        fixRecommendation:
            '''Please file a bug at https://github.com/sankofatech/sankofa/issues/new with the logs for this command.''',
      );
    }
...
Reason: Exited with code $exitCode.''',
  ...
  /// the `sankofaEnv` override is active so the version gate checks the
  /// correct revision.
  ///
  /// No-op on older Flutter pins that don't support `--sankofa-trace`
  ...
  Future<void> prepareBuildTrace({required String platform}) async {
    ...
    final revision = sankofaEnv.flutterRevision;
    final flutterVersion = await sankofaFlutter.resolveFlutterVersion(
      revision,
    );
    ...
    final traceFile = File(
      p.join(
        sankofaEnv.buildDirectory.path,
        'sankofa',
        'debug',
  ...
  /// Emits a flow-start event (`ph: "s"`) on the sankofa_cli tracer
  /// tied to the spawned flutter process's real pid. When flutter builds
  /// with `--sankofa-trace`, it records a flow-end with its own pid as
  ...
  void _emitFlutterSpawnFlow(Process flutter) {
    sankofaTracer.addSpawnFlowStart(id: flutter.pid, at: clock.now());
  }
  ...
  /// Sankofa itself spent around the Flutter build, subtracting Flutter's
  ...
  void writeBuildTraceSummary() {
    ...
    // derive Sankofa's overhead. Second pass (below) then bakes overhead
    ...
    final sankofaOverhead = totalElapsed - flutterBuild;
    ...
    final environment = BuildEnvironment.detect(
      ...
      projectRoot: sankofaEnv.getSankofaProjectRoot(),
    );
    ...
    final summary = BuildTraceSummary.fromEvents(
      events,
      platform: buildPlatform,
      sankofaOverhead: sankofaOverhead.isNegative
          ? Duration.zero
          : sankofaOverhead,
  ...
  /// Sankofa's fork of Flutter) with a try/finally that runs
  /// `flutter pub get` with the system installation of Flutter to reset
  /// `.dart_tool/package_config.json` to the system Flutter.
  Future<void> _runSankofaBuildCommand(SankofaBuildCommand command) async {
    try {
      await command();
    } finally {
  ...
  /// run by sankofa update the package_config.json file to point to
  /// sankofa's version of Flutter, which confuses VS Code. See
  /// https://github.com/sankofatech/sankofa/issues/1101 for more info.
...
''');
  ...
  Future<File> buildElfAotSnapshot({
    ...
    required SankofaArtifact genSnapshotArtifact,
    ...
    // Record a span on the sankofa_cli row so gen_snapshot time shows up
    ...
    final exitCode = await sankofaTracer.span<int>(
      name: 'gen_snapshot',
      category: 'subprocess',
      body: () => process.stream(
        sankofaArtifacts.getArtifactPath(artifact: genSnapshotArtifact),
        arguments,
        // Never run in shell because we always have a fully resolved
        // executable path.
        runInShell: false,
