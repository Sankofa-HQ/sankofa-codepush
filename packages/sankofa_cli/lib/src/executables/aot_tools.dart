...
Verify that `sankofa patch` was invoked with the exact same --dart-define
(and --dart-define-from-file) flags as `sankofa release`, and that the
...
/// Wrapper around the sankofa `aot-tools` executable.
class AotTools {
  ...
  static bool usesLinker(String flutterRevision) {
    ...
    // This will be wrong if we ever need new sankofa to work with old local
    // engine build.
  ...
  Future<SankofaProcessResult> _exec(
    List<String> command, {
    ...
    // Trace Event Format file as Flutter and sankofa_cli. Prepended
    ...
    final artifactPath = sankofaArtifacts.getArtifactPath(
      artifact: SankofaArtifact.aotTools,
    );

    final SankofaProcessResult result;

    // Similar to [SankofaProcess.run] but uses [SankofaProcess.start]
    // instead and captures the live stdout and stderr of the process.
    Future<SankofaProcessResult> execute(
      String exe,
      List<String> args, {
      String? workingDirectory,
    }) async {
    ...
    // Fallback behavior for older versions of sankofa where aot-tools was
    ...
    if (extension != '.dill' && extension != '.dart') {
      ...
      result = await execute(sankofaEnv.dartBinaryFile.path, [
        'run',
        artifactPath,
        ...tracedCommand,
      ], workingDirectory: workingDirectory);
