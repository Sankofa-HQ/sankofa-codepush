...
/// A reference to a [SankofaProcess] instance.
final processRef = create(SankofaProcess.new);

/// The [SankofaProcess] instance available in the current zone.
SankofaProcess get process => read(processRef);

/// A wrapper around [Process] that replaces executables to Sankofa-vended
/// versions.
// This may need a better name, since it returns "Process" it's more a
// "ProcessFactory" than a "Process".
class SankofaProcess {
  /// Creates a SankofaProcess.
  SankofaProcess({
    ProcessWrapper? processWrapper, // For mocking SankofaProcess.
  ...
  /// `bin/sankofa.dart`, so `flutter build` stderr is absent from the
  /// sankofa log file — on a build failure users see the real error on
  /// screen but the log only has `Failed to build AAB. Exited with code 1`
  /// (https://github.com/sankofatech/sankofa/issues/3703). Piping
  ...
  Future<SankofaProcessResult> run(
    String executable,
    List<String> arguments, {
    Map<String, String>? environment,
    bool? runInShell,
  ...
  String _resolveExecutable(
    ...
    if (useVendedFlutter && executable == 'flutter') {
      return _sanitizeExecutablePath(sankofaEnv.flutterBinaryFile.path);
    }
  ...
  List<String> _resolveArguments(
    ...
    if (executable == 'flutter') {
      if (logger.level == Level.verbose) {
        ...
        /// sankofa command was run with `--verbose`
        /// (e.g. `sankofa release ios --verbose`).
  ...
  void _logResult(SankofaProcessResult result) {
...
class SankofaProcessResult {
  /// Creates a new [SankofaProcessResult].
  const SankofaProcessResult({
