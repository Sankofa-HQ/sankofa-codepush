...
import 'package:sankofa_cli/src/extensions/version.dart';
...
/// A reference to a [SankofaFlutter] instance.
final sankofaFlutterRef = create(SankofaFlutter.new);

/// The [SankofaFlutter] instance available in the current zone.
SankofaFlutter get sankofaFlutter => read(sankofaFlutterRef);

/// {@template sankofa_flutter}
/// Helps manage the Flutter installation used by Sankofa.
/// {@endtemplate}
class SankofaFlutter {
  /// {@macro sankofa_flutter}
  const SankofaFlutter();
  ...
  String _workingDirectory({String? revision}) {
    revision ??= sankofaEnv.flutterRevision;
    return p.join(sankofaEnv.flutterDirectory.parent.path, revision);
  }
  ...
  /// Gradle error (see sankofatech/sankofa#3783). The user is directed
  /// to run `sankofa cache clean` to start over.
  Future<void> installRevision({required String revision}) async {
    ...
    try {
      // Clone the Sankofa Flutter repo into the target directory.
      await git.clone(
        url: flutterGitUrl,
    ...
    final SankofaProcessResult result;
    try {
  ...
  /// Returns the current Sankofa Flutter version and revision.
  ...
  /// Returns the current Sankofa Flutter version.
  /// Throws a [ProcessException] if the version check fails.
  /// Returns `null` if the version check succeeds but the version cannot be
  /// parsed.
  Future<String?> getVersionString() async {
    final flutterRevision = sankofaEnv.flutterRevision;
    return getVersionForRevision(flutterRevision: flutterRevision);
  }

  /// The current Sankofa Flutter version as a [Version]. Returns null if the
