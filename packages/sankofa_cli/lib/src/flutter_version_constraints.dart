...
/// allowlist of specific Sankofa-fork engine revisions below the floor
/// that also satisfy the rule.
///
/// Sankofa ships its own Flutter fork, and a single upstream Flutter
/// version can back multiple Sankofa-fork engine revisions. When a
/// feature first lands in a Sankofa-fork revision of version N before
...
/// stops mattering.
///
/// Append a hash to [allowedRevisions] every time Sankofa re-ships the
...
class FlutterSupportConstraint {
  ...
  /// Sankofa-fork engine revisions below [minVersion] that also satisfy
  /// this constraint.
...
/// on Apple Silicon. The arm64 upload was added in sankofatech/flutter#129
...
/// Flutter support for `flutter build --sankofa-trace=<path>` for emitting
/// Chrome Trace Event Format build traces.
///
/// Added in sankofatech/flutter#116. `minVersion` is set to the next
/// minor past the latest Sankofa Flutter release (currently 3.41.6), so
...
final buildTraceSupportConstraint = FlutterSupportConstraint(
  minVersion: Version(3, 41, 7),
  allowedRevisions: {
    // Current Sankofa Flutter pin (bin/internal/flutter.version). Can
