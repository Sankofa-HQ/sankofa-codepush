...
import 'package:sankofa_cli/src/archive/directory_archive.dart';
...
class CodePushClientWrapper {
  ...
  Future<App> createApp({required int organizationId, String? appName}) async {
    ...
    if (appName == null) {
      final defaultAppName = sankofaEnv.getPubspecYaml()?.name;
...
This app may not exist or you may not have permission to view it.''');
  ...
  void ensureReleaseIsNotActive({
    ...
    if (release.platformStatuses[platform] == ReleaseStatus.active) {
      final uri = SankofaWebConsole.appReleaseUri(release.appId, release.id);
      logger.err(
        '''
It looks like you have an existing ${platform.name} release for version ${lightCyan.wrap(release.version)}.
Please bump your version number and try again.

You can manage this release in the ${link(uri: uri, message: 'Sankofa Console')}''',
      );
      throw ProcessExit(ExitCode.software.code);
    }
  }
...
Please create a release using "sankofa release" and try again.
''');
      throw ProcessExit(ExitCode.software.code);
    }
...
Please run `sankofa cache clean` and try again. If the issue persists, please
file a bug report at https://github.com/sankofatech/sankofa/issues/new.
...
Looked in:
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib''',
    ...
    // (https://github.com/sankofatech/sankofa/issues/3388).
...
Sankofa looked for libapp.so under ${archsDir.path} but every requested
...
configuration excludes every architecture Sankofa was asked to build. Either
relax those filters or pass `--target-platform=<archs>` to restrict Sankofa
...
aar artifact already exists, continuing...''');
  ...
  /// Registers and uploads macOS release artifacts to the Sankofa server.
  ...
  /// Publishes a patch to the Sankofa server. This consists of creating a
  ...
  Never _handleErrorAndExit(
    ...
    if (error is CodePushUpgradeRequiredException) {
      progress?.fail();
      logger
        ..err('Your version of sankofa is out of date.')
        ..info(
          '''Run ${lightCyan.wrap('sankofa upgrade')} to get the latest version.''',
        );
