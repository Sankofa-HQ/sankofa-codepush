...
import 'package:sankofa_cli/src/deployment_track.dart';
...
void main() {
  group(PreviewCommand, () {
    ...
    Future<void> setupAndroidSankofaYaml(Invocation invocation) async {
      File(
          p.join(
    ...
    Future<File> sankofaYamlFileFromAab(File aab) async {
      final tempDir = Directory.systemTemp.createTempSync();
      final aabDirectory = Directory(p.join(tempDir.path, 'app-release'))
        ..createSync(recursive: true);
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.getSankofaYaml()).thenReturn(null);
      when(
      ...
      group(
        ...
        () {
          setUp(() {
            ...
            ).thenAnswer(setupAndroidSankofaYaml);
      ...
      group('setChannelOnAab', () {
        ...
        group('when channel is not set', () {
          group('when target channel is  production', () {
            test('does not change sankofa.yaml', () async {
              ...
              final updatedSankofaYamlFile = await sankofaYamlFileFromAab(
                aabFile,
              );
              expect(
                updatedSankofaYamlFile.readAsStringSync(),
                'app_id: $appId\n',
              );
            });
          });
          ...
          group('when target channel is not production', () {
            test('sets sankofa.yaml channel to target channel', () async {
              aabFile = await createAabFile(channel: null);
              await runWithOverrides(
                () =>
              ...
              expect(updatedSankofaYamlFile.readAsStringSync(), '''
app_id: $appId
channel: live
''');
            });
...
''');
      ...
      group('when unable to find sankofa.yaml', () {
        test('exits with code 70', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.software.code));
          verify(
            () => progress.fail('Exception: Unable to find sankofa.yaml'),
          ).called(1);
        });
      });
      ...
      group('when in a sankofa project without flavors', () {
        setUp(() {
          when(
            () => sankofaEnv.getSankofaYaml(),
          ).thenReturn(const SankofaYaml(appId: 'test-app-id'));
      ...
      group('when in sankofa project with flavors', () {
        setUp(() {
          when(() => sankofaEnv.getSankofaYaml()).thenReturn(
            const SankofaYaml(
              appId: 'test-app-id',
    ...
    group('ios', () {
      ...
      group('when install/launch succeeds (production)', () {
        late File sankofaYaml;
        setUp(() {
          sankofaYaml = setupIOSSankofaYaml();
          when(
            () => iosDeploy.installAndLaunchApp(
              bundlePath: any(named: 'bundlePath'),
              deviceId: any(named: 'deviceId'),
        ...
        test('exits with code 0', () async {
          ...
          expect(sankofaYaml.readAsStringSync(), equals('app_id: $appId'));
        });
      });
...
'''),
    ...
    group('linux', () {
      ...
      setUp(() {
        ...
        when(() => sankofaProcess.run(any(), any())).thenAnswer(
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () =>
                sankofaProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
      ...
      group('when preview artifact is cached', () {
        ...
        test('launches cached artifact', () async {
          ...
          verify(() => sankofaProcess.run('chmod', any())).called(1);
          verify(
            () =>
                sankofaProcess.start(any(that: endsWith(executableName)), []),
          ).called(1);
    ...
    group('macos', () {
      ...
      group('staging', () {
        ...
        setUp(() {
          ...
          sankofaYaml = setupMacosSankofaYaml();
        });

        test('sets channel in sankofa.yaml', () async {
          final result = await runWithOverrides(command.run);
          expect(result, equals(ExitCode.success.code));
...
2025-01-31 10:15:29.808 Df macos_sandbox[33557:2cc12] [dev.sankofa:updater::updater] [sankofa] Reporting successful launch.
'''
                .split('\n');

        setUp(() {
          setupMacosSankofaYaml();
        ...
        test('removes noisy lines from log output', () async {
          ...
          verify(
            () => logger.info(
              '''[dev.sankofa:updater::updater] [sankofa] Reporting successful launch.''',
            ),
          ).called(1);
          verifyNever(
            () => logger.info(
      ...
      group('non-UTF8 handling', () {
        ...
        test('handles non-UTF8 bytes in log stream without throwing', () async {
          ...
          final invalidUtf8Bytes = [
            ...utf8.encode(
              '2025-01-31 09:53:22.889 Df app[22535:1d268] [sankofa] Hello',
            ),
            0xFF,
            0x0A, // newline
          ];
          ...
          verify(() => logger.info('[sankofa] Hello\uFFFD')).called(1);
        });
      });
    });
    ...
    group('windows', () {
      ...
      void createSankofaYaml({String? channel}) {
        final sankofaYaml =
            File(
                p.join(
      ...
      group('when preview artifact is not cached', () {
        ...
        test('downloads and launches artifact', () async {
          ...
          verify(
            () => sankofaProcess.start(any(that: endsWith('runner.exe')), []),
          ).called(1);
          verify(() => logger.info('hello world')).called(1);
          verify(() => logger.err('hello error')).called(1);
        });
