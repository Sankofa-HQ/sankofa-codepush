...
void main() {
  group(ReleaseCommand, () {
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.usesSankofaCodePushPackage).thenReturn(true);
      ...
      when(
        () =>
            sankofaFlutter.installRevision(revision: any(named: 'revision')),
      ).thenAnswer((_) async => {});
      when(
...
✅ Published Release ${release.version}!'''),
        ...
        () => logger.info(
          '''To create a patch for this release, run ${lightCyan.wrap('sankofa patch --platforms=android --release-version=${release.version}')}''',
        ),
      ]);
    });
...
✅ Published Release ${release.version}!'''),
          () => logger.info(postReleaseInstructions),
          () => logger.info(
            '''To create a patch for this release, run ${lightCyan.wrap('sankofa patch --platforms=android --flavor=$flavor --target=$target --release-version=${release.version}')}''',
          ),
        ]);
      });
    ...
    group('when there is an existing release with the same version', () {
      ...
      group('when the release uses a different version of Flutter', () {
        setUp(() {
          ...
          when(
            () => sankofaFlutter.getVersionForRevision(
              flutterRevision: flutterRevision,
            ),
    ...
    group('when flutter-version is provided', () {
      ...
      test('fetches remote refs before resolving', () async {
        ...
        when(
          () => sankofaFlutter.resolveFlutterRevision(any()),
        ).thenAnswer((_) async => revision);

        await runWithOverrides(command.run);

        verify(() => sankofaFlutter.fetchRemoteRefs()).called(1);
      });
      ...
      group('when flutter version is supported', () {
        ...
        test('uses specified flutter version to build '
          ...
          when(() => releaser.buildReleaseArtifacts()).thenAnswer((_) async {
            ...
            expect(sankofaEnv.flutterRevision, equals(revision));
            return File('');
          });
        ...
        group('when flutter version install fails', () {
          setUp(() {
            when(
              () => sankofaFlutter.installRevision(
