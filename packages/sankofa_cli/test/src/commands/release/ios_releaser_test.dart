...
import 'package:sankofa_cli/src/commands/release/ios_releaser.dart';
    ...
    group('buildReleaseArtifacts', () {
      ...
      group('when not codesigning', () {
        ...
        test('logs warning about patching', () async {
          ...
          verify(
            () => logger.warn(
              '''sankofa preview will not work for releases created with "--no-codesign". However, you can still preview your app by signing the generated .xcarchive in Xcode.''',
            ),
          ).called(1);
          verify(
            () => logger.warn(
      ...
      group('when stale build/ios/sankofa directory exists', () {
        late Directory sankofaSupplementDir;

        setUp(() {
          sankofaSupplementDir = Directory(
            p.join(projectRoot.path, 'build', 'ios', 'sankofa'),
          )..createSync(recursive: true);
          when(
            () => artifactManager.getIosReleaseSupplementDirectory(),
          ).thenReturn(sankofaSupplementDir);
        });

        test('deletes the directory', () async {
          expect(sankofaSupplementDir.existsSync(), isTrue);
          await runWithOverrides(iosReleaser.buildReleaseArtifacts);
          expect(sankofaSupplementDir.existsSync(), isFalse);
        });
      });
    ...
    group('updatedReleaseMetadata', () {
      ...
      const metadata = UpdateReleaseMetadata(
        ...
        environment: BuildEnvironmentMetadata(
          ...
          usesSankofaCodePushPackage: true,
        ),
      );
...
If left checked, Xcode will rewrite the build number in the uploaded IPA, so the version that ships will not match the one Sankofa recorded for this release, and patches will fail to apply.
'''),
          );
        });
      });
