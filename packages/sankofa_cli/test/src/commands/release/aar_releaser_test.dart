...
import 'package:sankofa_cli/src/commands/release/aar_releaser.dart';
...
void main() {
  group(AarReleaser, () {
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.androidPackageName).thenReturn(packageName);
    ...
    group('minimumFlutterVersion', () {
      test('is null', () {
        // Sankofa has always had aar support, so we don't need to
    ...
    group('assertPreconditions', () {
      group('when validation succeeds', () {
        ...
        group('when androidPackageName is null', () {
          setUp(() {
            when(() => sankofaEnv.androidPackageName).thenReturn(null);
          });
    ...
    group('assembleSupplementDirectory', () {
      ...
      setUp(() {
        ...
        supplementDir = Directory(
          p.join(projectRoot.path, 'build', 'android', 'sankofa'),
        )..createSync(recursive: true);
    ...
    group('uploadReleaseArtifacts', () {
      ...
      setUp(() {
        when(
          () => sankofaAndroidArtifacts.extractAar(
...
1. Add the aar repo and Sankofa's maven url to your app's settings.gradle:
...
dependencyResolutionManagement {
    ...
    repositories {
        ...
        mavenCentral()
+       maven {
+           url '../${p.basename(sankofaEnv.getSankofaProjectRoot()!.path)}/${p.relative(p.join(projectRoot.path, 'release'))}'
+       }
+       maven {
-           url 'https://storage.googleapis.com/download.flutter.io'
+           url 'https://download.sankofa.dev/download.flutter.io'
+       }
    }
}
''')}
...
dependencies {
  // ...
  releaseImplementation '${sankofaEnv.androidPackageName}:flutter_release:$buildNumber'
  // ...
}''')}
'''),
        );
