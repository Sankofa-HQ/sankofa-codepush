...
import 'package:sankofa_cli/src/archive_analysis/android_archive_differ.dart';
...
void main() {
  group(AndroidPatcher, () {
    ...
    group('buildPatchArtifact', () {
      ...
      // See https://github.com/sankofatech/updater/issues/211
...
Looked in:
  ...
  - build/app/intermediates/stripped_native_libs/{flavor}Release/out/lib'''),
          ...
          test('prints message directing users to github issue', () async {
            ...
            verify(
              () => logger.info(
                any(
                  that: contains(
                    'https://github.com/sankofatech/sankofa/issues/2532',
                  ),
                ),
              ),
            ).called(1);
    ...
    group('extractReleaseVersionFromArtifact', () {
      setUp(() {
        when(
          () => sankofaAndroidArtifacts.extractReleaseVersionFromAppBundle(
            any(),
          ),
      ...
      test(
        '''returns value of sankofaAndroidArtifacts.extractReleaseVersionFromAppBundle''',
        () async {
          expect(
            await runWithOverrides(
