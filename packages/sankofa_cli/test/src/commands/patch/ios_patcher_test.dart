...
import 'package:sankofa_cli/src/archive_analysis/apple_archive_differ.dart';
...
import 'package:sankofa_cli/src/executables/aot_tools.dart';
import 'package:sankofa_cli/src/executables/xcodebuild.dart';
...
void main() {
  group(IosPatcher, () {
    ...
    setUpAll(() {
      ...
      registerFallbackValue(SankofaArtifact.genSnapshotIos);
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.iosSupplementDirectory).thenReturn(
        Directory(p.join(projectRoot.path, 'build', 'sankofa', 'ios')),
      );
      when(() => sankofaEnv.iosPodfileLockHash).thenReturn(null);
...
This may indicate that the patch contains native changes, which cannot be applied with a patch. Proceeding may result in unexpected behavior or crashes.''',
            ...
            group('when native diffs are not allowed', () {
              group('when in an environment that accepts user input', () {
                setUp(() {
                  when(() => sankofaEnv.canAcceptUserInput).thenReturn(true);
                });
    ...
    group('buildPatchArtifact', () {
      ...
      setUp(() {
        when(
          () => sankofaFlutter.getVersionAndRevision(),
        ).thenAnswer((_) async => flutterVersionAndRevision);
        when(
          () => sankofaFlutter.getVersion(),
...
For more information see: ${supportedFlutterVersionsUrl.toLink()}'''),
        ...
        group('when extraBuildArgs has obfuscation flags', () {
          ...
          test('includes obfuscation flags in build args', () async {
            ...
            patcher.extraBuildArgs = [
              ...
              '--split-debug-info=build/sankofa/symbols',
            ];
            await runWithOverrides(patcher.buildPatchArtifact);
    ...
    group('createPatchArtifacts', () {
      ...
      void setUpProjectRootArtifacts() {
        ...
        File(
          p.join(projectRoot.path, 'build', 'ios', 'sankofa', 'App.ct.link'),
        ).createSync(recursive: true);
        File(
          p.join(
            projectRoot.path,
      ...
      group('when uses linker', () {
        ...
        setUp(() {
          final sankofaRoot = Directory.systemTemp.createTempSync();
          flutterDirectory = Directory(
            p.join(sankofaRoot.path, 'bin', 'cache', 'flutter'),
          );
          ...
          when(
            () => sankofaArtifacts.getArtifactPath(
              artifact: SankofaArtifact.analyzeSnapshotIos,
            ),
...
<dict>
	...
	<dict>
		...
		<string>com.sankofa.timeShift</string>
        ...
        test('returns correct metadata', () async {
          const metadata = CreatePatchMetadata(
            ...
            environment: BuildEnvironmentMetadata(
              ...
              sankofaYaml: SankofaYaml(appId: 'app-id'),
              usesSankofaCodePushPackage: false,
            ),
          );
