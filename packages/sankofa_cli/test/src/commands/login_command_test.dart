...
import 'package:sankofa_cli/src/commands/login_command.dart';
...
void main() {
  group(LoginCommand, () {
    ...
    test('has correct description', () {
      expect(command.description, 'Login as a new Sankofa user.');
    });
    ...
    test('exits with code 70 if no user is found', () async {
      ...
      verify(
        () => logger.err('We could not find a Sankofa account for $email.'),
      ).called(1);
      verify(
        () => logger.info(any(that: contains('console.sankofa.dev'))),
      ).called(1);
    });
