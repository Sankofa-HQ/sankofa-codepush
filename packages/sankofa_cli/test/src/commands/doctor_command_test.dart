...
void main() {
  group('doctor', () {
    ...
    const sankofaFlutterRevision = 'test-flutter-revision';
    ...
    setUp(() {
      ...
      doctor = MockDoctor();
      gradlew = MockGradlew();
      logsDirectory = Directory.systemTemp.createTempSync('sankofa_logs');
      java = MockJava();
      ...
      ).thenReturn(sankofaFlutterRevision);
      when(() => sankofaEnv.logsDirectory).thenReturn(logsDirectory);
      when(() => doctor.initAndDoctorValidators).thenReturn([validator]);
      when(
        () => doctor.runValidators(any(), applyFixes: any(named: 'applyFixes')),
      ).thenAnswer((_) async => {});
      when(sankofaFlutter.getVersionString).thenAnswer((_) async => null);

      command = runWithOverrides(DoctorCommand.new)
        ..testArgResults = argResults;
    });

    test('prints sankofa version, flutter revision, '
        'and engine revision '
...
Sankofa v$packageVersion • git@github.com:sankofatech/sankofa.git
Flutter • revision ${sankofaEnv.flutterRevision}
...
'''),
    ...
    test(
      '''prints sankofa version, flutter revision, flutter version, and engine revision''',
      () async {
...
Sankofa v$packageVersion • git@github.com:sankofatech/sankofa.git
Flutter $flutterVersion • revision ${sankofaEnv.flutterRevision}
Engine • revision $sankofaEngineRevision
'''),
        ).called(1);
...
'''),
    ...
    group('when --json is passed', () {
      ...
      test('emits JSON success with version info and diagnostics', () async {
        ...
        expect(data['flutter_revision'], equals(sankofaFlutterRevision));
