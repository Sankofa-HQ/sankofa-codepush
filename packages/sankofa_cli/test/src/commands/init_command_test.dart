...
environment:
  sdk: ">=2.19.0 <3.0.0"''';
    ...
    setUp(() {
      ...
      when(
        () => sankofaEnv.getSankofaYamlFile(cwd: any(named: 'cwd')),
      ).thenReturn(sankofaYamlFile);
      when(
        () => sankofaEnv.getPubspecYamlFile(cwd: any(named: 'cwd')),
      ).thenReturn(pubspecYamlFile);
      when(() => sankofaEnv.getFlutterProjectRoot()).thenReturn(projectRoot);
      when(
...
'''),
    ...
    test('throws software error when pubspec.yaml is malformed.', () async {
      ...
      when(() => sankofaEnv.hasPubspecYaml).thenThrow(exception);
      final exitCode = await runWithOverrides(command.run);
      verify(
    ...
    test('throws software error when sankofa.yaml already exists', () async {
      when(() => sankofaEnv.hasSankofaYaml).thenReturn(true);
      final exitCode = await runWithOverrides(command.run);
      verify(
        () => logger.err(
          'A "sankofa.yaml" file already exists and seems up-to-date.',
        ),
      ).called(1);
      verify(
        () => logger.info(
    ...
    test(
      ...
      () async {
        ...
        when(() => sankofaYaml.appId).thenReturn(appId);
        when(() => sankofaYaml.flavors).thenReturn(null);
    ...
    test(
      ...
      () async {
        ...
        when(() => sankofaEnv.getSankofaYaml()).thenReturn(sankofaYaml);
        when(() => sankofaYaml.appId).thenReturn(appId);
        when(() => sankofaYaml.flavors).thenReturn({'dev': 'id-1'});
        when(
    ...
    test('does not prompt for name when unable to accept user input', () async {
      when(() => sankofaEnv.canAcceptUserInput).thenReturn(false);
    ...
    test('--force overwrites existing sankofa.yaml', () async {
      when(() => sankofaEnv.hasSankofaYaml).thenReturn(true);
      when(() => argResults['force']).thenReturn(true);
      final exitCode = await runWithOverrides(command.run);
      verifyNever(
      ...
      verify(
        () => sankofaYamlFile.writeAsStringSync(
    ...
    group('on non MacOS', () {
      ...
      test('creates sankofa for an android-only app', () async {
        final exitCode = await runWithOverrides(command.run);
        expect(exitCode, equals(ExitCode.success.code));
        verify(
          () => sankofaYamlFile.writeAsStringSync(
      ...
      test('creates sankofa for an app without flavors', () async {
        when(
          () => gradlew.productFlavors(any()),
      ...
      test('creates sankofa for an app with flavors', () async {
...
flavors:
  ...
  stable: ${appIds[1]}'''),
    ...
    test('creates sankofa.yaml for an app without flavors', () async {
      await runWithOverrides(command.run);
      verify(
        () => sankofaYamlFile.writeAsStringSync(
          any(that: contains('app_id: $appId')),
    ...
    group('creates sankofa.yaml for an app with flavors', () {
  ...
  prod: test-appId-prod'''),
      ...
      group('with new flavors added', () {
        ...
        setUp(() {
          ...
          when(() => sankofaEnv.getSankofaYaml()).thenReturn(sankofaYaml);
          when(() => sankofaYaml.appId).thenReturn(appId);
          when(() => sankofaYaml.flavors).thenReturn(existingFlavors);
        });
        ...
        test('creates new flavor entries in sankofa.yaml', () async {
...
flavors:
  ...
  d: test-appId-4'''),
    ...
    test('detects existing sankofa.yaml in pubspec.yaml assets', () async {
...
''');
    ...
    test('ensures that addSankofaYamlToPubspecAssets is called', () async {
      when(() => sankofaEnv.pubspecContainsSankofaYaml).thenReturn(false);
      await runWithOverrides(command.run);
