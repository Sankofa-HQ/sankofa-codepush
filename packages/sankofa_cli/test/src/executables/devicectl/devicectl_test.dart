...
void main() {
  group(Devicectl, () {
    ...
    setUp(() {
      ...
      processResult = MockSankofaProcessResult();
      devicectl = Devicectl();
      ...
      group('when install succeeds', () {
        ...
        group('when output json contains app bundleId', () {
          test("returns installed app's bundleId", () async {
            ...
            expect(bundleId, 'dev.sankofa.ios-test');
          });
        });
      });
    });
            ...
            expect(
              ...
              throwsA(
                isA<DevicectlException>().having(
                  (e) => '${e.underlyingException}',
                  'underlyingException',
                  '''Exception: Unable to launch dev.sankofa.ios-test because the device was not, or could not be, unlocked.''',
                ),
              ),
            );
          },
        ...
        test('returns exit code 70 ', () async {
          ...
          verify(
            () => progress.fail(
              any(
                that: contains(
                  '''Unable to launch dev.sankofa.ios-test because the device was not, or could not be, unlocked.''',
                ),
              ),
            ),
          ).called(1);
