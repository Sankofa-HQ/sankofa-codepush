...
void main() {
  group(IOSDeploy, () {
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.sankofaRoot).thenReturn(tempDir);
      when(
        () => sankofaEnv.flutterDirectory,
    ...
    group('installAndLaunchApp', () {
      ...
      test('executes correct command when deviceId is provided', () async {
        ...
        verify(
          () => sankofaProcess.start(any(that: endsWith('ios-deploy')), [
            '--debug',
            '--id',
            deviceId,
            '-r',
      ...
      test('exits with code 70 on process exception', () async {
        ...
        when(() => sankofaProcess.start(any(), any())).thenThrow(exception);
        const bundlePath = 'test-bundle-path';
      ...
      test('handles process resuming', () async {
        ...
        verify(
          () => progress.update(
            '''[  7%] Copying /Users/felix/Development/github.com/sankofatech/_sankofa/sankofa/bin/cache/previews/b3be02c7-97a8-4135-be71-814cc43d096e/ios_1.0.4+7.app/Base.lproj/Main.storyboardc/BYZ-38-t0r-view-8bC-Xf-vdC.nib to device''',
          ),
        ).called(1);
      });
    ...
    group('installIfNeeded', () {
      test('does nothing if ios-deploy is already installed', () async {
        ...
        verifyNever(() => sankofaProcess.run(any(), any()));
      });
      ...
      test(
        ...
        () async {
          when(() => sankofaProcess.run(any(), any())).thenAnswer((_) async {
            runWithOverrides(
              () => IOSDeploy.iosDeployExecutable.createSync(recursive: true),
            );
          ...
          verify(
            () => sankofaProcess.run('flutter', ['precache', '--ios']),
          ).called(1);
