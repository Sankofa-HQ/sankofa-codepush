...
import 'package:sankofa_cli/src/auth/sankofa_oauth.dart';
...
String _buildTestJwt({String issuer = 'https://auth.sankofa.dev'}) {
...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => sankofaEnv.jwtIssuer,
    ).thenReturn('https://auth.sankofa.dev');
  });
  ...
  group('obtainCredentialsViaLoopbackLogin', () {
    ...
    final authBaseUrl = Uri.parse('https://auth.sankofa.dev');
    ...
    test('constructs correct login URL with continue parameter', () async {
      ...
      expect(loginUri.host, equals('auth.sankofa.dev'));
    ...
    test('handles authBaseUrl with trailing slash', () async {
      final authBaseUrlWithSlash = Uri.parse('https://auth.sankofa.dev/v1/');
      final testJwt = _buildTestJwt(issuer: 'https://auth.sankofa.dev/v1/');
      when(
        () => sankofaEnv.jwtIssuer,
      ).thenReturn('https://auth.sankofa.dev/v1/');
    ...
    test('throws when redirect contains error parameter', () async {
      await expectLater(
        ...
        throwsA(
          isA<SankofaAuthException>().having(
            (e) => e.message,
            'message',
  ...
  group('refreshSankofaCredentials', () {
    late MockHttpClient httpClient;
    final authBaseUrl = Uri.parse('https://auth.sankofa.dev');
  ...
  group('SankofaAuthException', () {
    test('toString includes message', () {
      const exception = SankofaAuthException('test error');
      expect(
        exception.toString(),
        equals('SankofaAuthException: test error'),
      );
    });
  });
}
