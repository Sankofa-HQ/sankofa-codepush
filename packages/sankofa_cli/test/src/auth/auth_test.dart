...
const sankofaJwtIssuer = 'https://auth.sankofa.dev';
...
void main() {
    ...
    test('creates instance with default constructor', () {
      final instance = runScoped(
        ...
        values: {
          ...
          sankofaEnvRef.overrideWith(SankofaEnv.new),
        },
      );
      expect(
  ...
  group('OauthAuthProvider', () {
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.jwtIssuer).thenReturn(sankofaJwtIssuer);
    });
    ...
    group('authProvider', () {
      ...
      group('when issuer is auth.sankofa.dev', () {
        setUp(() {
          when(() => payload.iss).thenReturn(sankofaJwtIssuer);
        });

        test('returns AuthProvider.sankofa', () {
          runWithOverrides(() {
            expect(
              jwt.authProvider,
              equals(AuthProvider.sankofa),
            );
          });
        });
      });
  ...
  group(Auth, () {
    ...
    const refreshToken = 'sankofa-token';
    ...
    const sankofaCiToken = CiToken(
      refreshToken: 'sb_rt_test',
      authProvider: AuthProvider.sankofa,
    );
    ...
    setUp(() {
      ...
      when(() => sankofaEnv.jwtIssuer).thenReturn(sankofaJwtIssuer);
      when(
        () => sankofaEnv.authServiceUri,
      ).thenReturn(Uri.parse('https://auth.sankofa.dev'));
      when(() => sankofaEnv.hostedUri).thenReturn(null);
    ...
    group('AuthenticatedClient', () {
      ...
      group('token', () {
        test('does not require an onRefreshCredentials callback', () {
          expect(
            () => AuthenticatedClient.token(
              token: ciToken,
              httpClient: httpClient,
              authServiceUri: Uri.parse('https://auth.sankofa.dev'),
              refreshCredentials:
                  (
                    clientId,
                    credentials,
        ...
        group('when refreshing the token fails', () {
          ...
          test('exits and logs correctly', () async {
            ...
            verify(
              () => logger.info(
                '''Try logging out with ${lightBlue.wrap('sankofa logout')} and logging in again.''',
              ),
            ).called(1);
        ...
        group('when token is Sankofa', () {
          test(
            'refreshes via Sankofa and uses new token',
            () async {
              when(() => httpClient.send(any())).thenAnswer(
                (_) async => http.StreamedResponse(
                  const Stream.empty(),
              ...
              expect(
                ...
                equals('Bearer $sankofaIdToken'),
              );
              verify(
                () => httpClient.post(
                  Uri.parse('https://auth.sankofa.dev/token'),
                  headers: any(named: 'headers'),
                  body: any(named: 'body'),
                ),
              ).called(1);
          ...
          group('when Sankofa refresh fails', () {
            late AuthenticatedClient client;
            setUp(() {
              when(
                () => httpClient.post(
      ...
      group('credentials', () {
        ...
        group('when expired credentials have Sankofa issuer', () {
          test(
            'refreshes via Sankofa and uses new token',
            () async {
              when(() => httpClient.send(any())).thenAnswer(
              ...
              final onRefreshCredentialsCalls = <oauth2.AccessCredentials>[];
              final expiredSankofaCredentials = oauth2.AccessCredentials(
                oauth2.AccessToken(
                  'Bearer',
                  'accessToken',
                  DateTime.now().subtract(const Duration(minutes: 1)).toUtc(),
                ...
                'sb_rt_old',
                [],
                idToken: sankofaIdToken,
              );

              final client = AuthenticatedClient.credentials(
                credentials: expiredSankofaCredentials,
                httpClient: httpClient,
                authServiceUri: Uri.parse('https://auth.sankofa.dev'),
                onRefreshCredentials: onRefreshCredentialsCalls.add,
              );
        ...
        group('when Sankofa credential refresh fails', () {
          late AuthenticatedClient client;
          setUp(() {
            when(
              () => httpClient.post(
    ...
    group('client', () {
      ...
      group('when SANKOFA_TOKEN is an API key', () {
        setUp(() {
          when(() => platform.environment).thenReturn(<String, String>{
            sankofaTokenEnvVar: 'sb_api_abc123',
          });
        });
        ...
        test('parses as API key and sets isAuthenticated', () {
          ...
          verify(
            () => logger.detail(
              '[env] $sankofaTokenEnvVar parsed as API key',
            ),
          ).called(1);
        });
      ...
      group('when token is invalid', () {
        setUp(() {
          when(() => platform.environment).thenReturn(<String, String>{
            sankofaTokenEnvVar: 'not a base64 string',
          });
        });
        ...
        test(
          ...
          () async {
            ...
            verifyNever(
              () => logger.detail(
                '[env] $sankofaTokenEnvVar parsed as legacy CiToken',
              ),
            );
          },
        );
...
''',
      ...
      test('returns an authenticated client '
        ...
        when(() => platform.environment).thenReturn(<String, String>{
          sankofaTokenEnvVar: ciToken.toBase64(),
        });
        auth = buildAuth();
        final client = auth.client;
        expect(client, isA<http.Client>());
        ...
        verify(
          () => logger.warn(
            'SANKOFA_TOKEN contains a legacy CI token from '
            '`sankofa login:ci`. '
            'This format is deprecated and will stop working in a future '
            'release. '
            'Create an API key at https://console.sankofa.dev instead.',
          ),
        ).called(1);
        verify(
          () => logger.detail(
  ...
  group('OauthValues', () {
    group('clientId', () {
      ...
      test('throws UnsupportedError for sankofa', () {
        expect(
          () => AuthProvider.sankofa.clientId,
          throwsA(
            isA<UnsupportedError>().having(
              (e) => e.message,
              'message',
              'Sankofa auth does not use a client ID',
            ),
          ),
        );
      });
    ...
    group('authEndpoints', () {
      test('throws UnsupportedError for sankofa', () {
        expect(
          () => AuthProvider.sankofa.authEndpoints,
