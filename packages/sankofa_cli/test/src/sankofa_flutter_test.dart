...
void main() {
  group(SankofaFlutter, () {
    ...
    late SankofaProcessResult versionProcessResult;
    late SankofaProcessResult precacheProcessResult;
    ...
    setUp(() {
      ...
      flutterDirectory = Directory(p.join(sankofaRoot.path, 'flutter'));
      git = MockGit();
      ...
      versionProcessResult = MockSankofaProcessResult();
      precacheProcessResult = MockSankofaProcessResult();
      sankofaFlutter = runWithOverrides(SankofaFlutter.new);
      ...
      when(() => sankofaEnv.flutterRevision).thenReturn(flutterRevision);
      when(
    ...
    group('precacheArgs', () {
      group('when running on macOS', () {
        ...
        test('includes ios in platform list', () async {
          expect(
            runWithOverrides(() => sankofaFlutter.precacheArgs),
            contains('--ios'),
          );
        });
      });
    ...
    group('getConfig', () {
      late SankofaProcessResult configProcessResult;
      ...
      group('when process exists with non-zero code', () {
        ...
        test('returns empty map', () {
          expect(runWithOverrides(sankofaFlutter.getConfig), isEmpty);
          verify(
...
All Settings:
  ...
  ''');
        ...
        test('returns correct config map', () {
          expect(
            runWithOverrides(sankofaFlutter.getConfig),
            equals({
              'enable-web': '(Not set)',
              'enable-linux-desktop': '(Not set)',
              'enable-macos-desktop': '(Not set)',
    ...
    group('getSystemVersion', () {
      test(
        ...
        () async {
          ...
          await expectLater(
            runWithOverrides(sankofaFlutter.getSystemVersion),
    ...
    group('getVersionAndRevision', () {
      group('when unable to determine version', () {
        ...
        test('returns unknown (<revision>)', () async {
          await expectLater(
            runWithOverrides(sankofaFlutter.getVersionAndRevision),
    ...
    group('resolveFlutterRevision', () {
        ...
        test(
          ...
          () async {
            final revision = await runWithOverrides(
              () => sankofaFlutter.resolveFlutterRevision('3.10.6'),
            );
      ...
      group('when input is a valid git hash that exists locally', () {
        ...
        test('returns full hash for full input', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterRevision(fullHash),
          );
        ...
        test('returns full hash for short input', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterRevision('deadbeef'),
          );
          expect(revision, equals(fullHash));
        });
      });
      ...
      group('when input is a valid git hash format but does not exist', () {
        ...
        test('returns null', () async {
          ...
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterRevision(validHash),
          );
      ...
      group('when input is not a valid git hash format', () {
        test('returns null for too-short hash', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterRevision('abc'),
          );
          expect(revision, isNull);
        });
        ...
        test('returns null for non-hex string', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterRevision('not-a-version'),
          );
          expect(revision, isNull);
        });
      });
    ...
    group('resolveFlutterVersion', () {
      group('when input is a semver version', () {
        test(
          ...
          () async {
            final revision = await runWithOverrides(
              () => sankofaFlutter.resolveFlutterVersion('3.10.6'),
            );
      ...
      group('when input is not a recognized commit hash', () {
        ...
        test('returns null', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterVersion('not-a-version'),
          );
          expect(revision, isNull);
        });
      });
      ...
      group('when input is a recognized commit hash', () {
        ...
        test('returns a parsed version', () async {
          final revision = await runWithOverrides(
            () => sankofaFlutter.resolveFlutterVersion('deadbeef'),
          );
    ...
    group('fetchRemoteRefs', () {
      test('fetches from remote', () async {
        ...
        await runWithOverrides(
          () => sankofaFlutter.fetchRemoteRefs(),
        );
    ...
    group('getRevisionForVersion', () {
      ...
      group('when process exits with non-zero code', () {
        ...
        test('returns null', () async {
          await expectLater(
            runWithOverrides(
              () => sankofaFlutter.getRevisionForVersion(version),
            ),
            completion(isNull),
          );
          verify(
...
$revision
    ...
    group('getVersionString', () {
      group('when process exits with non-zero code', () {
        ...
        test('throws ProcessException', () async {
          await expectLater(
            runWithOverrides(sankofaFlutter.getVersionString),
            throwsA(isA<ProcessException>()),
          );
          verify(
            () => git.forEachRef(
    ...
    group('getVersion', () {
      group('when getVersionString returns null', () {
        ...
        test('returns null', () {
          expect(
            runWithOverrides(sankofaFlutter.getVersion),
            completion(isNull),
          );
        });
      });
...
origin/flutter_release/3.10.6''';
        ...
        await expectLater(
          runWithOverrides(sankofaFlutter.getVersions),
    ...
    group('installRevision', () {
      ...
      test('does nothing if the revision is already installed', () async {
        ...
        await runWithOverrides(
          () => sankofaFlutter.installRevision(revision: revision),
        );
      ...
      test('throws exception if unable to clone', () async {
        ...
        verify(
          () => git.clone(
            url: SankofaFlutter.flutterGitUrl,
      ...
      group('when precache throws', () {
        ...
        test(
          'throws CacheCorruptedException directing to sankofa cache clean',
          () async {
            await expectLater(
              runWithOverrides(
                () => sankofaFlutter.installRevision(revision: revision),
    ...
    group('isPorcelain', () {
      test('returns true when status is empty', () async {
        await expectLater(
          runWithOverrides(() => sankofaFlutter.isUnmodified()),
          completion(isTrue),
        );
        verify(
          () => git.status(
