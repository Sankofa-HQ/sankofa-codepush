...
import 'package:sankofa_cli/src/logging/logging.dart' hide logger;
...
void main() {
  group(SankofaCliCommandRunner, () {
    const sankofaEngineRevision = 'test-engine-revision';
    ...
    late SankofaCliCommandRunner commandRunner;
    ...
    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        ...
        values: {
          ...
          sankofaVersionRef.overrideWith(() => sankofaVersion),
        },
      );
    }
    ...
    setUp(() {
      ...
      when(
        () => sankofaFlutter.getVersionString(),
      ).thenAnswer((_) async => flutterVersion);
      when(sankofaVersion.isLatest).thenAnswer((_) async => true);
      when(sankofaVersion.isTrackingStable).thenAnswer((_) async => true);
      commandRunner = runWithOverrides(SankofaCliCommandRunner.new);
    });
    ...
    test('handles UsageException', () async {
      ...
      verify(
        () => logger.info(
          any(that: contains('Usage: sankofa <command> [arguments]')),
        ),
      ).called(1);
    });
...
Engine • revision $sankofaEngineRevision'''),
        ).called(1);

        // Making sure the only thing that was logged was the version info.
        // https://github.com/sankofatech/sankofa/issues/2260
    ...
    group('update check', () {
      group('when running upgrade command', () {
        setUp(() {
          ...
          when(
            sankofaVersion.fetchCurrentGitHash,
          ).thenAnswer((_) async => 'current');
          when(
            sankofaVersion.fetchLatestGitHash,
          ).thenAnswer((_) async => 'current');
        });
        test('does not check for update', () async {
          ...
          expect(result, equals(ExitCode.success.code));
          verifyNever(() => sankofaVersion.isTrackingStable());
          verifyNever(() => sankofaVersion.isLatest());
        });
      });
      ...
      group('when tracking the stable branch', () {
        ...
        test(
          ...
          () async {
            when(sankofaVersion.isLatest).thenThrow(Exception('error'));
            final result = await runWithOverrides(
              () => commandRunner.run(['--version']),
            );
            expect(result, equals(ExitCode.success.code));
        ...
        group('when update is available', () {
          test('logs update message', () async {
            ...
            verify(
              () => logger.info('A new version of sankofa is available!'),
            ).called(1);
            verify(
              () => logger.info(
                'Run ${lightCyan.wrap('sankofa upgrade')} to upgrade.',
              ),
            ).called(1);
      ...
      group('when not tracking the stable branch', () {
        setUp(() {
          when(
            sankofaVersion.isTrackingStable,
    ...
    group('--json', () {
      ...
      group('on parse-time errors', () {
        test('emits JSON error envelope on unknown command', () async {
          ...
          final error = json['error'] as Map<String, dynamic>;
          expect(error['code'], equals('usage_error'));
          expect(error['hint'], equals('Run: sankofa --help'));
          final meta = json['meta'] as Map<String, dynamic>;
          expect(meta['command'], equals('sankofa'));
...
class _TestCommand extends SankofaCommand {
...
class _ThrowingCommand extends SankofaCommand {
  @override
...
class _PromptRequiredCommand extends SankofaCommand {
...
/// [ansiOutputEnabled]).
class _CaptureModeCommand extends SankofaCommand {
