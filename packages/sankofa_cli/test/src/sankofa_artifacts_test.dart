import 'dart:io';

import 'package:mocktail/mocktail.dart';
import 'package:path/path.dart' as p;
import 'package:scoped_deps/scoped_deps.dart';
import 'package:sankofa_cli/src/cache.dart';
import 'package:sankofa_cli/src/engine_config.dart';
import 'package:sankofa_cli/src/sankofa_artifacts.dart';
import 'package:sankofa_cli/src/sankofa_env.dart';
import 'package:test/test.dart';

import 'mocks.dart';

void main() {
  group(SankofaCachedArtifacts, () {
    const engineRevision = 'engine-revision';
    late Cache cache;
    late Directory flutterDirectory;
    late Directory artifactDirectory;
    late SankofaEnv sankofaEnv;
    late SankofaCachedArtifacts artifacts;

    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        () => body(),
        values: {
          cacheRef.overrideWith(() => cache),
          sankofaEnvRef.overrideWith(() => sankofaEnv),
        },
      );
    }

    late Directory sankofaRoot;

    setUp(() {
      cache = MockCache();
      final tmpDir = Directory.systemTemp.createTempSync();
      sankofaRoot = tmpDir;
      flutterDirectory = Directory(p.join(tmpDir.path, 'flutter'))
        ..createSync(recursive: true);
      artifactDirectory = Directory(p.join(tmpDir.path, 'artifacts'))
        ..createSync(recursive: true);
      sankofaEnv = MockSankofaEnv();
      artifacts = const SankofaCachedArtifacts();

      when(
        () => cache.getArtifactDirectory(any()),
      ).thenReturn(artifactDirectory);
      when(() => sankofaEnv.flutterDirectory).thenReturn(flutterDirectory);
      when(() => sankofaEnv.sankofaRoot).thenReturn(sankofaRoot);
      when(
        () => sankofaEnv.sankofaEngineRevision,
      ).thenReturn(engineRevision);
    });

    group('getArtifactPath', () {
      test('returns the bundled aot_tools.dart path under sankofaRoot', () {
        expect(
          runWithOverrides(
            () =>
                artifacts.getArtifactPath(artifact: SankofaArtifact.aotTools),
          ),
          equals(
            p.join(
              sankofaRoot.path,
              'third_party',
              'aot_tools',
              'bin',
              'aot_tools.dart',
            ),
          ),
        );
      });

      test('returns correct path for iOS gen_snapshot', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotIos,
            ),
          ),
          equals(
            p.join(
              flutterDirectory.path,
              'bin',
              'cache',
              'artifacts',
              'engine',
              'ios-release',
              'gen_snapshot_arm64',
            ),
          ),
        );
      });

      test('returns correct path for macOS arm64 gen_snapshot', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotMacosArm64,
            ),
          ),
          equals(
            p.join(
              flutterDirectory.path,
              'bin',
              'cache',
              'artifacts',
              'engine',
              'darwin-x64-release',
              'gen_snapshot_arm64',
            ),
          ),
        );
      });

      test('returns correct path for macOS x64 gen_snapshot', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotMacosX64,
            ),
          ),
          equals(
            p.join(
              flutterDirectory.path,
              'bin',
              'cache',
              'artifacts',
              'engine',
              'darwin-x64-release',
              'gen_snapshot_x64',
            ),
          ),
        );
      });

      test('returns correct path for analyze_snapshot on iOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.analyzeSnapshotIos,
            ),
          ),
          equals(
            p.join(
              flutterDirectory.path,
              'bin',
              'cache',
              'artifacts',
              'engine',
              'ios-release',
              'analyze_snapshot_arm64',
            ),
          ),
        );
      });

      test('returns correct path for analyze_snapshot on macOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.analyzeSnapshotMacOS,
            ),
          ),
          equals(
            p.join(
              flutterDirectory.path,
              'bin',
              'cache',
              'artifacts',
              'engine',
              'darwin-x64-release',
              'analyze_snapshot',
            ),
          ),
        );
      });
    });
  });

  group(SankofaLocalEngineArtifacts, () {
    late String localEngineSrcPath;
    late String localEngine;
    late EngineConfig engineConfig;
    late SankofaLocalEngineArtifacts artifacts;

    R runWithOverrides<R>(R Function() body) {
      return runScoped(
        () => body(),
        values: {engineConfigRef.overrideWith(() => engineConfig)},
      );
    }

    setUp(() {
      localEngineSrcPath = 'local_engine_src_path';
      localEngine = 'local_engine';
      engineConfig = MockEngineConfig();
      artifacts = const SankofaLocalEngineArtifacts();

      when(
        () => engineConfig.localEngineSrcPath,
      ).thenReturn(localEngineSrcPath);
      when(() => engineConfig.localEngine).thenReturn(localEngine);
    });

    group('getArtifactPath', () {
      test('returns correct path for aot tools', () {
        expect(
          runWithOverrides(
            () =>
                artifacts.getArtifactPath(artifact: SankofaArtifact.aotTools),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'flutter',
              'third_party',
              'dart',
              'pkg',
              'aot_tools',
              'bin',
              'aot_tools.dart',
            ),
          ),
        );
      });

      test('returns correct path for gen_snapshot on iOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotIos,
            ),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'out',
              localEngine,
              'clang_x64',
              'gen_snapshot_arm64',
            ),
          ),
        );
      });

      test('returns correct path for arm64 gen_snapshot on macOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotMacosArm64,
            ),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'out',
              localEngine,
              'artifacts_arm64',
              'gen_snapshot',
            ),
          ),
        );
      });

      test('returns correct path for x64 gen_snapshot on macOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.genSnapshotMacosX64,
            ),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'out',
              localEngine,
              'artifacts_x64',
              'gen_snapshot',
            ),
          ),
        );
      });

      test('returns correct path for analyze_snapshot on iOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.analyzeSnapshotIos,
            ),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'out',
              localEngine,
              'clang_x64',
              'analyze_snapshot_arm64',
            ),
          ),
        );
      });

      test('returns correct path for analyze_snapshot on macOS', () {
        expect(
          runWithOverrides(
            () => artifacts.getArtifactPath(
              artifact: SankofaArtifact.analyzeSnapshotMacOS,
            ),
          ),
          equals(
            p.join(
              localEngineSrcPath,
              'out',
              localEngine,
              'clang_x64',
              'analyze_snapshot',
            ),
          ),
        );
      });
    });
  });
}
