...
void main() {
  group('SankofaProcess', () {
    const flutterStorageBaseUrlEnv = {
      'FLUTTER_STORAGE_BASE_URL': 'https://download.sankofa.dev',
    };
    ...
    late SankofaProcessResult runProcessResult;
    ...
    setUp(() {
      ...
      sankofaProcess = runWithOverrides(
        () => SankofaProcess(processWrapper: processWrapper),
      );

      when(
        () => sankofaEnv.flutterBinaryFile,
    ...
    test('SankofaProcessResult can be instantiated as a const', () {
      expect(
        () => const SankofaProcessResult(exitCode: 0, stdout: '', stderr: ''),
        returnsNormally,
      );
    });
    ...
    group('run', () {
      ...
      test('can override runInShell', () async {
        await runWithOverrides(
          () => sankofaProcess.run('git', ['pull'], runInShell: true),
        );
      ...
      test('forwards non-flutter executables to Process.run', () async {
        await runWithOverrides(
          () => sankofaProcess.run('git', ['pull'], workingDirectory: '~'),
        );
      ...
      test('sanitizes executable on windows', () async {
        ...
        await runWithOverrides(
          () => sankofaProcess.run(executable, ['--version']),
        );
        verify(
          () => processWrapper.run(
            '"$executable"',
      ...
      test('adds local-engine arguments if set', () async {
        ...
        sankofaProcess = SankofaProcess(processWrapper: processWrapper);

        await runWithOverrides(() => sankofaProcess.run('flutter', []));
    ...
    group('runSync', () {
      ...
      test('forwards non-flutter executables to Process.runSync', () async {
        runWithOverrides(
          () =>
              sankofaProcess.runSync('git', ['pull'], workingDirectory: '~'),
        );
      ...
      test('sanitizes executable on windows', () {
        ...
        const executable =
            r'C:\Program Files\Android\Android Studio\jbr\bin\java.exe';
        runWithOverrides(
          () => sankofaProcess.runSync(executable, ['--version']),
        );
        verify(
          () => processWrapper.runSync(
            '"$executable"',
      ...
      test('replaces "flutter" with our local flutter', () {
        runWithOverrides(
          () => sankofaProcess.runSync('flutter', [
            '--version',
          ], workingDirectory: '~'),
        );
      ...
      group('when log level is verbose', () {
        ...
        test('passes --verbose to flutter executable', () {
          runWithOverrides(() => sankofaProcess.runSync('flutter', []));
    ...
    group('stream', () {
      ...
      test('proxies to start with correct mode', () async {
        await expectLater(
          runWithOverrides(() => sankofaProcess.stream('git', ['pull'])),
    ...
    group('start', () {
      ...
      test('forwards non-flutter executables to Process.run', () async {
        await runWithOverrides(() => sankofaProcess.start('git', ['pull']));
      ...
      test('can override runInShell', () async {
        await runWithOverrides(
          () => sankofaProcess.start('git', ['pull'], runInShell: true),
        );
      ...
      test('sanitizes executable on windows', () async {
        ...
        const executable =
            r'C:\Program Files\Android\Android Studio\jbr\bin\java.exe';
        await runWithOverrides(
          () => sankofaProcess.start(executable, ['--version']),
        );
        verify(
          () => processWrapper.start(
            '"$executable"',
      ...
      test('replaces "flutter" with our local flutter', () async {
        await runWithOverrides(
          () => sankofaProcess.start('flutter', ['run']),
        );
