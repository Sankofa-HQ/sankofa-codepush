...
void main() {
  const sankofaEngineRevision = '8b89f8bd9fc6982aa9c4557fd0e5e89db1ff9986';
  const manifest = ArtifactsManifest(
    flutterEngineRevision: 'ec975089acb540fc60752606a3d3ba809dd1528b',
    storageBucket: 'download.sankofa.dev',
    artifactOverrides: {
  ...
  group('artifactProxy', () {
    ...
    test('should return 404 when no manifest is found', () async {
      const path =
          'flutter_infra_release/flutter/$sankofaEngineRevision/android-x64-release/artifacts.zip';
      when(() => client.getManifest(any())).thenThrow(Exception('oops'));
      final request = buildRequest(path);
      final response = await handler(request);
      expect(response.statusCode, equals(HttpStatus.notFound));
      verify(() => client.getManifest(sankofaEngineRevision)).called(1);
    });
    ...
    test('should proxy to Sankofa artifacts '
    ...
    test('should proxy to Flutter artifacts '
      ...
      const path =
          'flutter_infra_release/flutter/$sankofaEngineRevision/windows-x64/font-subset.zip';
      final request = buildRequest(path);
      final response = await handler(request);
      expect(
        response,
    ...
    test('should return 404 '
      ...
      const path =
          'flutter_infra_release/flutter/$sankofaEngineRevision/unknown/artifacts.zip';
      final request = buildRequest(path);
      final response = await handler(request);
      expect(response.statusCode, equals(HttpStatus.notFound));
      verifyNever(() => client.getManifest(sankofaEngineRevision));
    });
    ...
    test('should return explainer at /', () async {
      ...
      expect(response.readAsString(), completion(contains('Sankofa')));
    });
  });
}
